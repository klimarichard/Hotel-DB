...
// value this validator refuses and the template becomes unsavable.
...
function customDefaultError(v: unknown): string | null {
  ...
  if (d.kind === "fixedVar") {
    if (typeof d.key !== "string" || d.key.length === 0) {
      return "u výchozí hodnoty není vybrána zabudovaná proměnná.";
    }
    if (d.key.length > CUSTOM_VAR_LABEL_MAX) {
      return "název zabudované proměnné ve výchozí hodnotě je příliš dlouhý.";
    }
    return null;
  }
 ...
 * it produced recited the entire accepted shape without saying which slot was
 * wrong — so a template that had drifted into an unsavable state (an image
 * choice with nothing uploaded, an over-long default, a condition operand with
 * no variable picked) could not be repaired: the author had no way to find the
 * offending field. Keep this in lockstep with the identical function in
 * dokumenty.ts — the two are deliberate mirrors.
 */
function variableDefsError(v: unknown): string | null {
  ...
  for (const [key, def] of Object.entries(v as Record<string, unknown>)) {
    if (!CUSTOM_VAR_KEYS.has(key)) {
      return `Neznámá proměnná „${key}“ – šablona nabízí pouze var1–var${CUSTOM_VAR_KEYS.size}.`;
    }
