...
export const APP_TOUR_STEPS: TourStep[] = [
  ...
  { anchor: "menu-footer", hideOnMobile: true, title: "Přihlášený uživatel", body: "V levém dolním rohu vidíte, kdo je právě přihlášen do aplikace.", placement: "right" },
  { anchor: "theme-toggle", hideOnMobile: true, title: "Světlý/tmavý režim", body: "Můžete si zvolit zobrazení ve světlém, nebo tmavém režimu. Aplikace si bude vaši volbu pamatovat i při příštím přihlášení.", placement: "right" },
  ...
  // ── Směny (/smeny) ──────────────────────────────────────────────────────────
  { section: SECTIONS.shifts, permission: "nav.shifts.view", anchor: "nav-smeny", mobileAnchor: "bottomnav-smeny", title: "Směny", body: "V sekci Směny najdete měsíční plán směn.", placement: "right" },
  ...
  { permission: "shifts.cells.editOwnX", excludeIfPermission: "shifts.cells.edit", addedInVersion: 11, anchor: "shift-rows", route: DEMO_SHIFTS, title: "Zadání vlastního volna (X)", body: "Do svého řádku můžete po vytvoření plánu směn na další měsíc přímo zadat volno (X). Volno můžete zadat dvojklikem na danou buňku. Maximální počet pro zaměstnance na plný úvazek je 8 X, pro poloviční úvazek 13 X. Pokud potřebujete více dní, můžete požádat o výjimku (po zadání devátého/čtrnáctého X se vám automaticky zobrazí okno s žádostí o X navíc, kam napíšete odůvodnění). Admin poté žádost schválí, nebo zamítne (také s odůvodněním). Pokud potřebujete zadat 7 a více dní volna v kuse, požádejte o dovolenou.", placement: "bottom" },
  ...
  // Spotlights the redesigned change-request modal itself. The modal is
  // auto-opened on the dedicated demo route
  // (scenario "shifts-change-request" → ShiftPlannerPage sets pendingChangeRequest
  // once the mock published plan loads), so the `shift-change-request-modal`
  // anchor (added to the modal container by another agent) is present.
  { permission: "shifts.changeRequest.submit", excludeIfPermission: "shifts.cells.edit", addedInVersion: 10, anchor: "shift-change-request-modal", route: DEMO_SHIFTS_CHANGE_REQUEST, title: "Žádost o změnu směny", body: "V publikovaném plánu směn můžete požádat o změnu dvojklikem na příslušnou buňku. Otevře se vám tento dialog, kde můžete vybrat typ směny, na který chcete svoji stávající směnu vyměnit, požádat o výměnu směny s kolegou (stačí požádat pouze jednou, není potřeba žádat podruhé u kolegy, se kterým chcete měnit), napsat počet hodin, který jste odpracovali (pokud jste např. museli odejít dřív), požádat o smazání, nebo napsat textový požadavek, pokud potřebujete požádat o něco jiného, než nabízí připravené možnosti.", placement: "left" },
  ...
  { permission: "shifts.counterTable.view", anchor: "shift-counter", route: DEMO_SHIFTS, title: "Tabulka obsazenosti", body: "Pod plánem je souhrnná tabulka obsazenosti - počty lidí na jednotlivých pozicích pro každý den. Pokud někdo odpracoval jen část směny (například 8 z 12 hodin), objeví se v buňce vykřičník. Kliknutím na buňku zbývající hodiny rozdělíte mezi další zaměstnance.", placement: "bottom" },
  ...
  // ── Dovolená (/dovolena) ─────────────────────────────────────────────────────
  { section: SECTIONS.vacation, permission: "nav.vacation.view", anchor: "nav-dovolena", mobileAnchor: "bottomnav-dovolena", title: "Dovolená", body: "Sekce Dovolená slouží k podávání a sledování žádostí o dovolenou.", placement: "right" },
  ...
  { permission: ["recepce.ambiance.odvody.manage", "recepce.superior.odvody.manage", "recepce.amigo.odvody.manage", "recepce.ankora.odvody.manage"], addedInVersion: 20, hideOnMobile: true, anchor: "odvody-open", route: DEMO_PROTOKOL, title: "Příprava odvodu", body: "V sekci Účty vedle tlačítka „+ Přidat účet“ najdete tlačítko „Připravit odvod“, kterým otevřete formulář odvodu za zvolený měsíc. Zadejte v něm počty bankovek (a mincí), které budete odvádět a zaškrtněte, které účty budete odvádět. Opište hodnoty z Protelu (CZK cash, CZK cash depozit, EUR cash, EUR cash depozit). Částky k odvedení se dopočítají samy.", placement: "bottom" },
  { permission: ["recepce.ambiance.protokol.view", "recepce.superior.protokol.view", "recepce.amigo.protokol.view", "recepce.ankora.protokol.view"], addedInVersion: 20, hideOnMobile: true, anchor: "odvody-settle", route: DEMO_PROTOKOL, title: "Provedení odvodu", body: "Uložený odvod přidá do Účtů zamčený řádek „odvod + účty“. Na poslední noční směně v měsíci se u něj objeví tlačítko Provést odvod – tím řádek z protokolu zmizí a EUR bankovky z trezoru připravené pro odvod se smažou. Tento krok je nevratný.", placement: "left" },
  ...
  { permission: ["recepce.ambiance.protokol.view", "recepce.superior.protokol.view", "recepce.amigo.protokol.view", "recepce.ankora.protokol.view"], addedInVersion: 12, hideOnMobile: true, anchor: "protokol-print", route: DEMO_PROTOKOL_SIGNED, title: "Tisk protokolu", body: "Jakmile je protokol podepsaný oběma recepčními, můžete ho vytisknout.", placement: "bottom" },
  ...
  // ── Recepce → Lobby bar (demo tab; jen Ambiance) ─────────────────────────────
  { permission: "recepce.ambiance.lobbyBar.view", addedInVersion: 13, hideOnMobile: true, anchor: "lobbybar-add", route: DEMO_LOBBY_BAR, title: "Prodej z lobby baru", body: "Tímto tlačítkem přidáte prodej: datum, položku z ceníku, počet kusů, měnu a kdo prodal. Cena, provize i částka do společné kasy se dopočítají samy.", placement: "bottom" },
  { permission: "recepce.ambiance.lobbyBar.view", addedInVersion: 13, hideOnMobile: true, anchor: "lobbybar-cenik", route: DEMO_LOBBY_BAR, title: "Ceník položek", body: "Vpravo je ceník. Každá položka má cenu v korunách i v eurech.", placement: "left" },
  ...
  // Gated on the bulk key alone – steps support OR, not AND, and the button also
  ...
  { section: SECTIONS.templates, permission: "nav.contractTemplates.view", anchor: "nav-smlouvy", mobileAnchor: "bottomnav-more", title: "Šablony smluv", body: "Zde se spravují šablony pracovních smluv a dodatků.", mobileBody: "Zde se spravují šablony pracovních smluv a dodatků. Na telefonu sekci otevřete přes záložku Více ve spodní liště.", placement: "right" },
  ...
  { hideOnMobile: true, permission: "nav.faktury.view", addedInVersion: 19, anchor: "faktury-new", route: "/faktury", title: "Vytvoření faktury", body: "Tlačítkem Nová faktura založíte fakturu. Číslo faktury opište z Protelu – podle něj aplikace sama pozná hotel a to, zda jde o zálohovou fakturu (hotel i typ ale můžete kdykoli přepsat ručně). Vyplníte údaje o hostovi, odběratele a jednotlivé řádky. Popis řádku vyberete z katalogu položek, který k němu rovnou doplní sazbu DPH. Pokud potřebujete vlastní text, sazbu DPH a typ řádku musíte zvolit sami. U zálohové faktury se v rekapitulaci DPH vypisují pouze sazby, na kterých je nějaká částka – nulové řádky se na ní netisknou. Platil-li host kartou, vložte do pole EFT účtenka účtenku z platebního terminálu zkopírovanou z faktury v Protelu – i se svislými čárkami, protože právě ty určují sloupce; na vytištěné faktuře se účtenka sama zarovná do sloupců. Tlačítkem Vytisknout otevřete fakturu jako PDF v nové záložce.", placement: "bottom" },
  ...
  { section: SECTIONS.alerts, permission: "nav.alerts.view", anchor: "nav-upozorneni", mobileAnchor: "bottomnav-more", title: "Upozornění", body: "Sekce Upozornění shromažďuje důležité termíny a úkoly ke schválení.", mobileBody: "Sekce Upozornění shromažďuje důležité termíny a úkoly ke schválení. Na telefonu ji otevřete přes záložku Více ve spodní liště.", placement: "right" },
  ...
  // ── Návody (/navody) ─────────────────────────────────────────────────────────────
  { section: SECTIONS.guides, permission: "nav.guides.view", addedInVersion: 14, anchor: "nav-navody", mobileAnchor: "bottomnav-more", title: "Návody", body: "V sekci Návody najdete návody a užitečné materiály k práci.", mobileBody: "V sekci Návody najdete návody a užitečné materiály k práci. Na telefonu ji otevřete přes záložku Více ve spodní liště.", placement: "right" },
  ...
  { permission: "settings.companies.manage", anchor: "settings-tab-seznamy", route: "/nastaveni", reveal: ["settings-tab-seznamy"], title: "Správa společností", body: "Na záložce Seznamy v sekci Společnosti přidáváte a upravujete údaje o firmách.", placement: "bottom" },
  { permission: "settings.departments.manage", anchor: "settings-tab-seznamy", route: "/nastaveni", reveal: ["settings-tab-seznamy"], title: "Správa oddělení", body: "Na záložce Seznamy v sekci Oddělení spravujete seznam oddělení.", placement: "bottom" },
  { permission: "settings.jobPositions.manage", anchor: "settings-tab-seznamy", route: "/nastaveni", reveal: ["settings-tab-seznamy"], title: "Správa pozic", body: "Na záložce Seznamy v sekci Pracovní pozice spravujete seznam pracovních pozic.", placement: "bottom" },
  { permission: "settings.educationLevels.manage", anchor: "settings-tab-seznamy", route: "/nastaveni", reveal: ["settings-tab-seznamy"], title: "Správa vzdělání", body: "Na záložce Seznamy v sekci Vzdělání spravujete úrovně dosaženého vzdělání.", placement: "bottom" },
