...
- **Recepce** — a permission-driven front-desk hub per hotel (Ambiance, Superior, Amigo & Alqush, Ankora): a shift handover protocol (cash counting, accounts, notes, signature-based handover), a walk-in sales log, and a taxi-ride log with commission tracking and a shared route price list; the handover protocol also prepares the month-end cash transfer to the bank (Odvody), counting banknotes out of the vault and till, clearing filed receipts, and working out what to book against each Protel register; shared-terminal user types can be configured so that logout requires a superior to authorize with their own password.
...
**Odvody** slouží k **měsíčnímu odvodu hotovosti z recepce do banky**. Nejde o samostatnou záložku: odvod se připravuje přímo z **Předávacího protokolu** tlačítkem **Připravit odvod** v sekci **Účty**, hned vedle tlačítka „+ Přidat účet".

Tlačítko vidí jen uživatelé s oprávněním **„Připravit odvod"** u daného hotelu (v matici oprávnění je zařazeno pod **Předávacím protokolem**, vedle „Spravovat protokol"). Běžná recepční ho nevidí. Na podepsaném protokolu se tlačítko nezobrazí – podpis je nutné nejdřív zrušit.

V modálu nahoře přepínáte **měsíc** šipkami; na hotel a měsíc připadá **jeden odvod**. Pod nadpisem je vždy vidět, **do kterého protokolu se odvod zapíše** – je to protokol právě probíhající směny, což nemusí být ten, který máte otevřený.

Formulář má šest částí:

1. **Nominály CZK** – u každé bankovky a mince zapíšete **počet kusů**, které jdou do banky. Sloupce **TREZOR** a **KASA** ukazují, kolik kusů je podle protokolu k dispozici. Bankovky se berou přednostně z trezoru; nestačí-li v něm daný nominál, zbytek se **automaticky dobere z kasy** a u řádku se to vypíše. Teprve nestačí-li ani součet obou, řádek zčervená.
...
5. **Poměr rozdělení** – jen u **Amigo & Alqush**. Určuje, v jakém poměru mají po odvodu zůstat zůstatky v obou pokladnách. Odpovídá **počtu pokojů** (výchozí 70 : 24), takže se prakticky nemění – je proto jen vypsaný, a upravíte ho tlačítkem **Upravit poměr**, které otevře malé okno s oběma hodnotami.
...
- odečtou se zadané **bankovky CZK** (z trezoru, a co v něm nestačí, z kasy),
- **zaškrtnuté účty** se z protokolu smažou,
- místo nich přibude jeden **zamčený řádek „odvod + účty"** v jejich součtu.

Celkový součet CZK v protokolu se tím **nezmění** – peníze se jen přesunou do jedné položky, protože z hotelu zatím fyzicky neodešly. **Bankovky EUR zůstávají na místě.**
