...
- **Contracts & templates** — a Word-like (TipTap) template editor with variables and conditional blocks; contracts are generated server-side as PDFs, with signing-date sanity warnings (weekends, public holidays, or a date later than the document's validity); standalone documents (Multisport, Hmotná odpovědnost, custom templates) can be bulk-generated for many employees at once (Hromadné generování), producing both per-employee files and one merged print-ready PDF, and auto-fill contract-linked variables from the employee's currently running contract.
...
- **Vacation** — request/approval workflow with automatic shift-collision handling, plus an hour-based yearly balance (nárok / čerpáno / zůstatek) on each employee's record that auto-fills from locked payroll periods and can be corrected by hand with the right permission; users can optionally be granted read-only access to their own balance on Můj profil (off by default).
...
- **Recepce** — a permission-driven front-desk hub per hotel (Ambiance, Superior, Amigo & Alqush, Ankora): a shift handover protocol (cash counting, accounts, notes, signature-based handover), a walk-in sales log, a taxi-ride log with commission tracking and a shared route price list, and a month-end cash-transfer sheet (Odvody) that counts banknotes out of the vault, clears filed receipts from the protocol, and works out what to book against each Protel register; shared-terminal user types can be configured so that logout requires a superior to authorize with their own password.
...
> ⚠️ **Nemůže se odhlásit sám:** U sdíleného účtu může mít administrátor navíc zapnutou volbu **„Nemůže se odhlásit sám – odhlášení musí potvrdit nadřízený heslem"**. Kliknutí na **Odhlásit** pak takový účet neodhlásí rovnou – místo toho se otevře okno **„Autorizovat odhlášení"** s textem: „Sdílený terminál nelze odhlásit. Pro přihlášení ke svému osobnímu účtu, prosím, použijte své zařízení nebo anonymní režim prohlížení.“ V okně zvolíte z rozbalovacího seznamu **Jméno** nadřízeného, který smí odhlášení autorizovat, a tento nadřízený zadá **své vlastní heslo**. Terminál se odhlásí až po potvrzení tlačítkem **Odhlásit**; tlačítkem **Zrušit** okno zavřete beze změny. Každou autorizaci systém zaznamená do **Logu změn** včetně jména osoby, která odhlášení potvrdila. Na mobilu funguje odhlášení stejně – tlačítko **Odhlásit** najdete v panelu **„Více"**.
...
Záložka **Odvody** slouží k **měsíčnímu odvodu hotovosti z recepce do banky**. Zobrazí se jen uživatelům s oprávněním **„Odvody"** u daného hotelu; zadávat a měnit odvod smí jen ti, kdo mají navíc **„Spravovat odvody"**. Běžná recepční tuto záložku nevidí.

Nahoře přepínáte **měsíc** šipkami; na hotel a měsíc připadá **jeden odvod**.

**Zadání odvodu** – tlačítkem **Zadat odvod** otevřete formulář, který má šest částí:

1. **Nominály CZK** – u každé bankovky a mince zapíšete **počet kusů**, které jdou z trezoru do banky. Sloupec **K dispozici** ukazuje, kolik kusů je podle protokolu v trezoru; zadáte-li víc, řádek zčervená.
2. **Nominály EUR** – totéž pro eura.
3. **Účty k odvedení** – zaškrtnete účty, které už jsou **zavedené v účetnictví**. Zaškrtnuté účty se z protokolu odstraní; nezaškrtnuté v něm zůstanou.
4. **Hodnoty z Protelu** – opíšete čtyři hodnoty: **CZK cash**, **CZK cash depozit**, **EUR cash** a **EUR cash depozit**. U hotelu **Amigo & Alqush** zadáváte **osm** hodnot, protože oba hotely mají v Protelu vlastní registry (i když sdílejí jednu hotovost).
5. **Poměr rozdělení** – jen u **Amigo & Alqush**. Určuje, v jakém poměru mají po odvodu zůstat zůstatky v obou pokladnách. Odpovídá **počtu pokojů** (výchozí 70 : 24) a lze jej změnit, kdyby se počty pokojů někdy změnily.
6. **Výpočet** – průběžně ukazuje, kolik se má z které položky odvést. Přepočítává se sám při každé změně.

Případná upozornění (například že depozity převyšují odváděnou částku) se zobrazí červeně, ale **uložení nezablokují** – rozhodnutí zůstává na vás.

**Co se stane po uložení** – do **předávacího protokolu právě probíhající směny** se zapíše, že peníze odešly z trezoru:

- z **trezoru CZK** se odečtou zadané bankovky,
- **zaškrtnuté účty** se z protokolu smažou,
- místo nich přibude jeden **zamčený řádek „odvod + účty"** v jejich součtu.

Celkový součet CZK v protokolu se tím **nezmění** – peníze se jen přesunou do jedné položky, protože z hotelu zatím fyzicky neodešly. **Bankovky EUR zůstávají v trezoru.**

**Provedení odvodu** – na **poslední noční směně v měsíci** se v protokolu u zamčeného řádku objeví tlačítko **Provést odvod**. Po potvrzení řádek z Účtů zmizí a z **trezoru EUR** se odečtou odváděné bankovky. Teprve tímto krokem peníze z protokolu odejdou. Tlačítko může stisknout **kterákoli recepční**, která smí upravovat protokol.

> ⚠️ **Provedený odvod už nelze upravit ani smazat.** Dokud proveden není, můžete jej tlačítkem **Upravit odvod** kdykoli přepsat nebo tlačítkem **Smazat odvod** zrušit – účty i bankovky se v takovém případě vrátí do protokolu zpět.

**Tisk** – tlačítko **Tisk** vytiskne podklad na jednu stranu A4: nadpis **ODVODY**, měsíc, čtyři částky (u Amigo & Alqush zvlášť za každý hotel) a řádek s termínem, do kdy je potřeba odvod provést.
