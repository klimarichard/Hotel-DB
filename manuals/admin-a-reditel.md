...
- Zaměstnanec aktuálně na rodičovské dovolené má ve sloupcích **Pozice** a **Oddělení** zobrazený odznak **„Rodičovská"**. Pracuje-li takový zaměstnanec zároveň na druhý úvazek (souběžná smlouva, viz níže), zobrazují se u jeho jména **oba odznaky smlouvy** (např. HPP i DPP) a ve sloupcích Pozice/Oddělení text **„RODIČOVSKÁ / [aktuální pozice]"**.
...
- **Zobrazované jméno:** ve formuláři úprav je hned pod poli **Jméno** a **Příjmení** nepovinné pole **„Zobrazované jméno"** – zkrácené nebo jinak upravené jméno (např. přezdívka), které se má u zaměstnance zobrazovat místo jeho úředního jména. Necháte-li pole prázdné, použije se běžné „Jméno Příjmení". Je-li vyplněné, použije se **všude v aplikaci** – i tam, kde se dřív zobrazovalo úřední jméno, například ve starších plánech směn, ve mzdách, v dovolených a v upozorněních. Výjimkou zůstávají **úřední dokumenty** (smlouvy, generovaná PDF, export do CSV) a **seznamy nebo výběrová pole řazená podle příjmení** (např. seznam zaměstnanců) – tam se záměrně dál používá úřední jméno, aby šlo řadit a vyhledávat podle příjmení.
...
Na detailu zaměstnance je – vedle sekcí **Osobní údaje**, **Benefity** apod. – sbalitelná sekce **„Dovolená – zůstatek"**. Ukazuje bilanci dovolené zaměstnance za kalendářní rok, **v hodinách**.

- **Přepínání roku:** v záhlaví sekce je přepínač roku – šipkami vlevo/vpravo se posunete o rok zpět nebo vpřed. Sekce se vždy otevře na **aktuálním roce**; dopředu se posunout nelze (budoucí rok ještě nemá co zobrazovat), zpět nejdále do roku 2022.
- **Rok bez údajů:** není-li pro zvolený rok zatím nic vyplněno, sekce to napíše. Údaje zadáte dvojklikem v tabulce pod hlášením – tím se záznam pro daný rok založí.
- **Souhrnný řádek** nad tabulkou zobrazuje za zvolený rok:
  - **Loňská** – dovolená převedená z loňského roku (v hodinách). *Upravitelné* (dvojklikem); může být i záporná, pokud zaměstnanec loni vyčerpal víc, než měl.
  - **Letošní** – letošní nárok na dovolenou (v hodinách). *Upravitelné* dvojklikem.
  - **Nárok** – součet **Loňská + Letošní**. Počítá se automaticky, **nelze ho upravit ručně**.
  - **Čerpáno** – kolik hodin dovolené už bylo v daném roce vyčerpáno.
  - **Zůstatek** – kolik hodin zbývá (Nárok mínus Čerpáno). Je-li záporný, zobrazí se **červeně**.
- **Tabulka čerpání po měsících** má dva řádky: měsíce **1–12** a na konci sloupec **CELKEM** se součtem za celý rok. V každém měsíčním sloupci je počet hodin dovolené vyčerpaných v daném měsíci.
- **Automatické vyplnění:** hodnota za měsíc se doplní sama ve chvíli, kdy je příslušné mzdové období v **Mzdách uzamčeno** – přebírá se z čerpání dovolené evidovaného tam. Dokud období uzamčené není, měsíc zůstává prázdný (pokud ho mezitím ručně nevyplní někdo s oprávněním níže).
- **Ruční úprava:** hodnoty v tabulce může upravovat pouze uživatel s oprávněním **„Upravit zůstatek dovolené (po měsících)"** (Zaměstnanci → Dovolená – zůstatek v matici oprávnění / Nastavení → Uživatelské typy). Buňku měsíce upravíte **dvojklikem** a zadáte počet hodin; necháte-li pole prázdné, hodnota se vymaže. Ručně upravená buňka je označená **hvězdičkou „*"** – stejně jako u ručních úprav ve **Mzdách**.
- Sekci **vidí** každý, kdo má přístup k evidenci zaměstnanců (stejně jako ostatní sekce detailu); **upravovat** hodnoty ale může jen ten, komu bylo přiděleno oprávnění výše.

> 📷 *(Místo pro snímek obrazovky: sekce Dovolená – zůstatek na detailu zaměstnance)*
...
- **PPP — Počet hodin týdně:** u Nástupu se smlouvou **PPP (zkrácený úvazek)** se zobrazí pole **„Počet hodin týdně"** — zadejte počet hodin, na které zaměstnanec nastupuje (např. `20`). Hodnota se promítne do vygenerované smlouvy a určuje, kolik dovolené zaměstnanec za měsíc nakumuluje (počítá se poměrně k úvazku). Nárok na Navíc (přesčas) vzniká až po překročení plného měsíčního základu — stejně jako u ostatních smluvních typů. Počet hodin lze kdykoli změnit přidáním Dodatku s druhem změny **„počet hodin"**.
...
- **Upozornění na mzdu pod minimem:** při zadávání nebo úpravě mzdy u Nástupu nebo mzdového Dodatku systém porovná zadanou částku s aktuální minimální mzdou. Je-li mzda nižší, zobrazí se **varování** — uložení přesto není blokováno. Kontrola platí pro: HPP plnou výší minimální mzdy; PPP poměrnou výší odpovídající počtu sjednaných hodin; DPP se nekontroluje.
- **Upozornění na datum podpisu:** připadne-li zadané **datum podpisu** na víkend nebo český státní svátek, zobrazí se ve formuláři varování *„Datum podpisu připadá na víkend nebo svátek."*. Jde jen o upozornění – uložení nijak neblokuje a může se zobrazit současně s existujícím varováním o datu podpisu pozdějším než datum platnosti.
...
- **Rodičovská dovolená:** tlačítkem **+ Rodičovská** (v záhlaví pracovního poměru, vedle **+ Dodatek** a **Ukončit smlouvu**) zaznamenáte období rodičovské dovolené. V okně **Rodičovská dovolená** zadáte **Začátek** a volitelně **Konec** — datum konce nemusíte znát předem a pole můžete nechat prázdné. Konec pak doplníte kliknutím na **✎** (upravit) přímo na pruhu Rodičovská; prázdný konec se zobrazuje jako **„dosud"**. Jde o čistě **informativní** záznam — **nemění mzdy, výpočty ani stav smlouvy**. Zobrazí se jako barevný pruh **„Rodičovská"** s daty přímo u příslušného pracovního poměru; odebrat jej lze křížkem **✕**. Po dobu, kdy rodičovská právě probíhá, se v **seznamu zaměstnanců** ve sloupcích **Pozice** a **Oddělení** zobrazí odznak **„Rodičovská"**.
  - **Souběžně může probíhat jen jedna rodičovská:** má-li zaměstnanec už rodičovskou dovolenou, která ještě neskončila (i s koncem zadaným do budoucna), je tlačítko **+ Rodičovská** do jejího ukončení **nedostupné (šedé)**. Najetím myší na tlačítko se zobrazí vysvětlení: *„Zaměstnanec už má rodičovskou dovolenou, která neskončila. Souběžně může probíhat jen jedna."*
- **Souběžná smlouva při rodičovské:** pracuje-li zaměstnanec na rodičovské zároveň na druhý úvazek (např. DPP), přidejte v historii pracovního poměru nový **Nástup** s typem tohoto druhého úvazku. Aplikace automaticky přiřadí mzdy i směny k naposledy zahájené aktivní smlouvě (DPP) a po jejím ukončení se vrátí zpět k hlavní smlouvě. V **seznamu zaměstnanců** se u takového zaměstnance zobrazují **oba odznaky smlouvy** (např. HPP i DPP) a ve sloupcích **Pozice** a **Oddělení** text **„RODIČOVSKÁ / [aktuální pozice]"**.
- U každého řádku lze podle stavu **vygenerovat smlouvu**, zobrazit ji, nebo nahrát podepsanou kopii.

##### Prohlášení poplatníka

Vedle tlačítka **+ Nástup** je v záhlaví této záložky tlačítko **Prohlášení poplatníka** – vygeneruje vyplněné **Prohlášení poplatníka daně** (formulář MFin 5457, PDF) a otevře ho v nové záložce prohlížeče.
...
> 📷 *(Místo pro snímek obrazovky: záložka Historie pracovního poměru s tlačítky + Nástup a Prohlášení poplatníka)*

Na této záložce tak zůstává jen samotná historie pracovního poměru – zakládání ad hoc dokumentů (Multisport, Hmotná odpovědnost apod.) se přesunulo na záložku **Další dokumenty**, popsanou níže.
...
> 📷 *(Místo pro snímek obrazovky: záložka Další dokumenty se společným seznamem dokumentů a tlačítkem + Generovat dokument)*
...
- **Vytvoření plánu, přechody mezi stavy, nastavení termínů a kopírování zaměstnanců z předchozího plánu** děláte vy (administrátor i ředitel). Při kopírování zaměstnanců se spolu s lidmi přenesou i jejich přiřazená písmena MOD (vedoucí směny) – dřív se kopíroval jen seznam zaměstnanců a písmena bylo nutné přiřazovat znovu. Pokud je v cílovém měsíci u některého vedoucího písmeno již přiřazené, kopírování ho nepřepíše.
...
- **Označení odpracovaných hodin typem směny:** zapíšete-li do buňky jen **počet odpracovaných hodin** (např. `8`), objeví se v rohu buňky malý **`+`**. Kliknutím na něj vyberete typ směny (`DA`, `DS`, … `NPA`), pod kterým byly hodiny odpracovány — buňka se pak započítá k tomuto typu v **tabulce obsazenosti**. **Na výši mzdy to nemá žádný vliv** (slouží jen pro přehled obsazení). U složeného zápisu jako `DA+2` je typ (DA) zřejmý a štítek se nezadává. Štítek (i to malé `+`) vidí jen uživatelé, kteří mohou vyplňovat směny ve **všech stavech** plánu.
...
- **MOD (vedoucí směny):** u jména FOM se zobrazuje písmeno MOD; písmeno upravíte kliknutím na odznak u jména. Toto přiřazení je nyní jediným místem, odkud se písmeno bere – dřív bylo u části vedoucích napevno dané v aplikaci, nově záleží výhradně na tom, co tady přiřadíte. Pokud v novém plánu u některého vedoucího žádné písmeno není, jednoduše ho přiřaďte kliknutím – při dalším kopírování zaměstnanců (viz výše) se pak přenese samo. U stávajících plánů zůstávají písmena beze změny, nic není třeba dělat. Počty směn u MOD (`MOD: N …`) se zobrazují jen ve stavech *Uzavřený* a *Publikovaný*.
...
V **publikovaném** plánu může zaměstnanec dvojklikem na vlastní buňku podat **žádost o změnu** směny. V dialogu zaměstnanec upřesní požadavek: změnu na jiný typ směny, výměnu s kolegou, zadání počtu odpracovaných hodin, smazání nebo vlastní textový popis (volba **„Jiné"**).

Po **schválení** se příslušná změna do plánu **propíše automaticky** — nemusíte ji provádět ručně. Výjimkou je volba **„Jiné"**: zaměstnanec ji vyplnil vlastním textem a odpovídající úpravu provedete vy v plánu ručně. Počet čekajících žádostí ukazuje odznak u položky *Směny* v menu.

### Export

Tlačítko **Exportovat ▾** nabízí **PDF** (plán na jednu stránku A4) a **CSV**; export do CSV nyní obsahuje i přiřazená písmena MOD u vedoucích směny. Tlačítkem **DNES** se vrátíte na aktuální měsíc.
...
- **Upozornění na datum podpisu:** připadne-li v okně generování zadané **datum podpisu** na víkend nebo český státní svátek, zobrazí se varování *„Datum podpisu připadá na víkend nebo svátek."*. Jde jen o upozornění – generování nijak neblokuje.
...
- Tlačítko **Nahrát podepsanou smlouvu ▾** nabízí dvě volby:
  - **Smlouva** – nahraje vybraný soubor celý jako podepsanou smlouvu, stejně jako dosud.
  - **Smlouva + prohlášení** – použijte, když máte podepsanou smlouvu naskenovanou dohromady s prohlášením poplatníka jako jeden soubor. Po výběru souboru se zobrazí okno, ve kterém zadáte, **kolik prvních stran** skenu tvoří smlouvu (obvykle `1`) – potvrdíte tlačítkem **Nahrát a rozdělit**. Aplikace sken automaticky rozdělí: první strany uloží jako podepsanou smlouvu a zbylé strany jako samostatný dokument **„Prohlášení poplatníka"** na záložce **Další dokumenty** (název dokumentu lze v okně před uložením upravit).
  - Po nahrání kopii smlouvy lze také **smazat** a vrátit záznam do stavu „nepodepsáno".
  - > 📝 Je-li nahrávaný soubor velký naskenovaný dokument, aplikace ho při nahrávání automaticky **zmenší**, aby zabíral méně místa – čitelnost zůstane zachována. Zmenšení platí pro obě volby nahrávání.
- Pokud po vygenerování změníte parametry řádku, nabídne se **Znovu vygenerovat smlouvu**.

> 📷 *(Místo pro snímek obrazovky: záložka Smlouvy s tlačítkem Nahrát podepsanou smlouvu a nabídkou Smlouva / Smlouva + prohlášení)*
...
- **Dovolená:** pod sekcí **Pracovní poměr** se může zobrazovat sbalitelná sekce **„Dovolená"** s vaší vlastní bilancí dovolené za kalendářní rok, v hodinách – stejné údaje jako v sekci **„Dovolená – zůstatek"** na detailu zaměstnance: přepínač roku (otevře se na aktuálním roce; dopředu se posunout nelze, zpět nejdále do roku 2022 – u roku bez údajů to sekce napíše); souhrn **Loňská** / **Letošní** / **Nárok** (= Loňská + Letošní) / **Čerpáno** / **Zůstatek** (záporný se zobrazí červeně); a tabulka čerpání po měsících **1–12** se sloupcem **CELKEM**. Ručně upravený měsíc je označen hvězdičkou **„*"** s vysvětlivkou *„Ručně upraveno"* po najetí myší. Hodnoty se doplňují automaticky, jakmile je příslušné mzdové období v Mzdách uzamčeno. Sekce je čistě **informativní** – nic v ní upravit nejde; případnou opravu provede administrátor na vaší kartě zaměstnance v *Zaměstnanci*. Zobrazí se pouze tomu, kdo má oprávnění **„Zobrazit vlastní zůstatek dovolené"** (*Můj profil* v matici oprávnění) – jde-li o typ **Administrátor**, má toto oprávnění vždy; ostatním typům, včetně **Ředitele**, ho musí administrátor přidělit ručně v *Nastavení → Uživatelské typy*.
...
- Přepínač **„Sdílený terminál"** je určen pro pracoviště, kde se z jednoho společného účtu střídá víc lidí – typicky recepce. Je-li zapnutý, zápisy provedené v sekci **Recepce** (protokol, Walkiny, Taxi) se v historii protokolu i v **Logu změn** nepřiřadí tomuto společnému účtu, ale osobě, která na protokolu **předchozí** směny podepsala **Převzal**. Uživatel přihlášený pod svým vlastním účtem (např. manažer nebo administrátor) se podepíše vždy sám za sebe. Nelze-li osobu ve směně určit – protokol pro předchozí směnu neexistuje nebo není podepsaný – použije se přihlášený účet.
- Přepínač **„Nemůže se odhlásit sám – odhlášení musí potvrdit nadřízený heslem"** se obvykle zapíná společně se **„Sdílený terminál"**. Účty daného typu se pak nemohou odhlásit samy – kliknutí na **Odhlásit** otevře okno **„Autorizovat odhlášení"**, kde odhlášení musí svým vlastním heslem potvrdit nadřízený (podrobnosti viz kapitola [Recepce](../README.md#recepce-uživatelská-příručka) v README). Kdo smí odhlášení autorizovat, určuje oprávnění **„Autorizovat odhlášení"** (sekce *Systém* v matici oprávnění) – administrátor (Superadmin) smí odhlášení autorizovat vždy, bez ohledu na toto oprávnění.
...
- Sekce **Recepce** v matici oprávnění obsahuje i dvě oprávnění pro nové záložky **Lobby bar** a **Terminál**: **„Ambiance – Spravovat lobby bar"** a **„Amigo & Alqush – Spravovat terminál"**. Obě jsou zanořená pod oprávnění pro zobrazení příslušné záložky – zaškrtnutím „Spravovat…" se automaticky zaškrtne i zobrazení záložky; odškrtnutím zobrazení se odškrtne i „Spravovat…". **„Spravovat lobby bar"** navíc odemyká nastavení viditelného období a editaci ceníku položek včetně sazeb provize; **„Spravovat terminál"** odemyká nastavení viditelného období a sloupec **Předáno**.
- Sekce **Můj profil** v matici oprávnění obsahuje oprávnění **„Zobrazit vlastní zůstatek dovolené"** (vedle „Odhalit vlastní citlivé údaje"). Řídí, zda se na stránce *Můj profil* zobrazuje sekce **„Dovolená"** s vlastní bilancí dovolené uživatele. **Ve výchozím stavu ho nemá žádný typ uživatele** – sekci tedy nikdo (mimo administrátora, který má vždy všechna oprávnění) automaticky nevidí, dokud ho typu ručně nepřidělíte.
...
Aplikace je primárně určena pro práci na počítači, ale otevřít ji lze i v mobilním prohlížeči. Na telefonu se rozložení přizpůsobí malé obrazovce:

- **Spodní lišta místo bočního menu:** boční menu vlevo se skryje a nahradí ho **lišta se záložkami u spodního okraje obrazovky**. Několik nejpoužívanějších sekcí je přímo jako záložky; ostatní (Zaměstnanci, Mzdy, Šablony smluv, Upozornění, Log změn, Nastavení) najdete pod záložkou **„Více"**. Tam jsou i tlačítka **světlý/tmavý režim**, **Nápověda** a **Odhlásit** a údaj o **verzi aplikace**.
- **Datově hutné stránky doporučujeme na počítači:** seznam zaměstnanců, mzdy a plán směn jsou nejpřehlednější na větší obrazovce. Na telefonu je lze zobrazit a posouvat, ale pro běžnou administrativní práci je vhodnější počítač.
