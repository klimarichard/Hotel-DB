...
| [Authentication, Roles & Permissions](docs/auth-and-permissions.md) | Login + password reset, the configurable permission model (catalogue + editable user types + per-user grants/revokes), `requirePermission` enforcement, the admin type/permission UI, per-role menu order, and the phone-only `mobilePermission` gating pattern. |
...
| [Recepce (Reception)](docs/recepce.md) | Permission-driven per-hotel hub (Ambiance/Superior/Amigo & Alqush/Ankora); Předávací protokol (cash/trezor counting, sm/sm-trezor/wata, virtual dual-signature handover, freeze-on-sign, element-level history + undo/redo); Walkiny and Taxi (visible-range gating, global taxi ceník, manager Provize total); recepce retention sweep; mobile-only `recepce.mobile.view` gating pattern. |
