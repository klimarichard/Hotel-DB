...
| [Contracts & Templates](docs/contracts.md) | Contract types, TipTap template editor, server-side PDF generation, template variables (incl. PPP `{{hoursPerWeek}}` / `{{isHalfTime}}` (v4.8.2), `{{newHoursPerWeek}}` / `{{isDodatekHodiny}}`, and `{{isMale}}`), editable variable overview at generation time, per-template custom variables (`{{var1}}`..`{{var10}}`, label+type+optional **default value (v4.8.2)** configured per template, never persisted), nested `{{#if}}`/`{{#unless}}` conditional blocks (v4.8.0 block-parser rewrite), in-editor + byte-accurate PDF template preview (v4.8.0), **one-step ad-hoc document generation (v4.8.2 — replaces the old row-first empty-PDF-row flow for new documents)**, **probation-aware termination-template selection + DPP fix (v4.8.2)**, signed-contract "Smlouva + prohlášení" split with client-side scan compression, de-duplicated boolean variables (single-key + `{{#unless}}`) and the deploy-order trap when changing the permanent variable catalogue. |
