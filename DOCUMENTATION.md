...
| [Shifts (Shift Planner)](docs/shifts.md) | Shift-expression parser, monthly grid, MOD badges, structured shift-change-request picker + auto-apply on approval, double-click X on open plans, X-limits, plan lifecycle, numeric-cell shift-type tagging (per-type tally). |
