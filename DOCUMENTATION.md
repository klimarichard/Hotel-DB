...
| [Recepce (Reception)](docs/recepce.md) | Permission-driven per-hotel hub (Ambiance/Superior/Amigo & Alqush/Ankora); Předávací protokol (cash/trezor counting, sm/sm-trezor/wata, virtual dual-signature handover, freeze-on-sign, element-level history + undo/redo, coalesced typing edits, single "created" floor entry); Walkiny and Taxi (visible-range gating, global taxi ceník, manager Provize total); shared-terminal write attribution (`sharedTerminal` roleType flag → `resolveRecepceActor`/`resolveOnDutyActor`, `viaUid`/`viaEmail`); recepce retention sweep; mobile-only `recepce.mobile.view` gating pattern. |
