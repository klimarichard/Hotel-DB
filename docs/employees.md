...
- **Hromadné generování (v4.12.0):** a "Hromadné generování" button next to "Exportovat CSV" on `EmployeesPage` opens `BulkGenerateModal` — generates the same standalone document (Hmotná odpovědnost, Multisport, or a custom standalone template) for many employees at once, orchestrated **client-side** as N ordinary single-document generation calls rather than through a new bulk endpoint. See [Contracts & Templates — Bulk standalone document generation](contracts.md#bulk-standalone-document-generation--hromadné-generování-v4120) for the full write-up, including why it stays client-orchestrated and the display-only `contracts.generate.bulk` permission.
...
The card header shows the session's **effective state** computed by folding every Dodatek's `changes[]` onto the Nástup: `jobTitle`, `contractType`, `salary` (with eye-toggle reveal), `companyId`, `startDate`, `endDate`. Header buttons: `+ Dodatek`, `Ukončit smlouvu`. `+ Dodatek` hides once the session is `terminated` — defined as either an explicit Ukončení row OR an effective `endDate` in the past (a fixed-term contract that ran out without anyone filing the termination paperwork). `Ukončit smlouvu` hides on `terminated` as well **as of v5.9.0** — an expired contract is already over, so offering to end it again read as a live action. ⚠️ The trade-off, reversing the v4.x decision: a lapsed fixed-term session can no longer be given a formal Ukončení row from this header. (History: originally both buttons hid on `terminated`; then `Ukončit smlouvu` was deliberately made existence-based (`!session.ukonceni`) so fixed-term cards could be ended retroactively; v5.9.0 moved it back behind `terminated` on request.) The related "no active employment" warning in `AddEntryModal` now treats a non-terminated session as active (`groupBySession(...).some(s => !s.terminated)`) instead of requiring `endDate === null`, so terminating a fixed-term contract no longer shows a spurious warning.

Each row inside the card carries: `Upravit` (hidden once a signed PDF is on file — editing then would silently desync from the legal document), the three-button contract action set, and `Smazat`. Page-level header button: `+ Nástup` (locked-changeType AddEntryModal).

> **v4.6.0 update:** the bottom collapsible **Adhoc smlouvy** section described below (and its `+ Adhoc dokument ▾` trigger) is gone — ad-hoc contracts now live in the Další dokumenty tab. See [Employee detail restructure — ad-hoc documents move to Další dokumenty (v4.6.0)](#employee-detail-restructure--ad-hoc-documents-move-to-další-dokumenty-v460) below. The historical description immediately below reflects the 2026-05-06 design and is kept for context.
...
- `computeEffectiveState(nastup, dodatky, ukonceni)` — folds Dodatek changes:
  ...
  - `úvazek (počet hodin)` (v5.9.0) → **both** `hoursPerWeek` (the number, from `value`) and `contractType` (HPP/PPP, stated outright in the new `ChangeRow.contractType` field). Each half applies independently, so a row carrying only one still folds. This kind merges the two below.
  - `úvazek` *(legacy)* → `contractType` via `uvazekToContractType()` ("poloviční"/"zkrácený"/"částečný" → PPP, "plný" → HPP, otherwise unchanged). Guessing from free text is exactly why the kinds were merged: unrecognised wording silently changed nothing, which payroll then read as "still full-time".
  - `počet hodin` *(legacy)* → `hoursPerWeek` only; never touched the contract type.
  - ⚠️ **No migration was run** — both legacy kinds stay stored, readable and editable forever. A legacy *pair* of rows folds to exactly the same state as one merged row (verified against the compiled helper). The Dodatek form re-offers a row's own retired kind as `"úvazek (původní)"`, because a `<select>` that cannot represent the stored value would erase it on save. The fold itself lives in **one helper per package** — `applyUvazekChange()` in `frontend/src/lib/changeKinds.ts` and `functions/src/services/changeKinds.ts` — because there are **six** fold sites and inline copies would drift.
...
- **`DELETE /api/employees/:id`** blocks the delete if the employee has **any** payroll or shift history — an entry in any `payrollPeriods/*/entries/{employeeId}`, membership in any `shiftPlans/*/planEmployees`, **a shift cell in any `shiftPlans/*/shifts`** (added v2.2.2: a cell can outlive its membership row, so checking membership alone let an employee with orphan-prone cells slip through and get hard-deleted), **or a manual hour split referencing them in any `shiftPlans/*/shiftSplits`** (same reasoning, added with feature/shift-hour-splits — matched via the `employeeIds` array-contains mirror field on the split doc, since Firestore cannot query inside the `entries` array of objects; see "Manual hour splits" in [Shifts](shifts.md)). Such employees must be **terminated** (`Ukončit smlouvu`), not deleted. Index-free by design: it iterates the (monthly) `payrollPeriods` / `shiftPlans` parents and uses a doc-id lookup / collection-scope query rather than a `collectionGroup` query (which would need a `COLLECTION_GROUP` index the emulator does **not** enforce → silent prod 500s). For the allowed (no-history) path the endpoint removes the employee's sub-collections + the Storage blobs for `contracts` and `otherDocuments`, the top-level `alerts` / `vacationRequests` / `employeeChangeRequests` / `probationAlerts`, the linked `users.employeeId` (unlink, or delete the account with `?deleteUser=true`), **and any stray shift-plan request docs** (`unavailabilityRequests` / `shiftOverrideRequests` / `shiftChangeRequests`, added v2.2.2). `auditLog` is intentionally preserved as immutable history.
...
- **Transition chain in the session header.** When a Dodatek shifts the `jobTitle` or `contractType` during a session, the header surfaces the full chronological chain — `recepční → senior recepční → Front Office Manager` for the title, `HPP → PPP → HPP` inside the contract-type tag chip. All entries before the last render with the existing `.titleFrom` strikethrough so they read as history; the trailing entry is the current value. `frontend/src/lib/employmentSessions.ts` exports `collectFieldChain(session, field)` which walks each Dodatek's `changes[]`, reads HPP/PPP from the merged kind's `contractType` field (or maps legacy `úvazek` free text via `uvazekToContractType()`), and collapses consecutive duplicates. Sessions with length-1 chains (no transitions) collapse to the plain current value.

---

## Employee detail restructure — ad-hoc documents move to Další dokumenty (v4.6.0)

Two buttons swapped tabs so each tab holds only what its name promises: **Historie pracovního poměru** now holds only employment history, and **Další dokumenty** holds every non-generated-contract document a person has, generated or uploaded alike.

- **"+ Adhoc dokument" → "+ Generovat dokument", moved to Další dokumenty.** The dropdown (built-in standalone types — Multisport, Hmotná odpovědnost — plus custom standalone templates) used to sit on the Historie toolbar; it now sits on the Další dokumenty toolbar. The contracts list and the `generateModal` state it drives still live on `EmployeeDetailPage` (which owns the employee's whole contracts fetch) — the button itself is injected into `OtherDocumentsTab` via a new **`toolbarSlot?: ReactNode` prop**, so the child component doesn't need its own copy of the generation state.
- **The separate "Ad hoc smlouvy" section is deleted.** `frontend/src/components/AdhocContractsSection.tsx` and its CSS module are gone. Ad-hoc contracts (`employmentRowId == null` — Multisport, Hmotná odpovědnost, custom standalone) are now **interleaved into the single Další dokumenty list** alongside uploaded documents, because from the user's point of view a generated Multisport form and an uploaded PDF scan are both just "another document on this person".
  - **Sort order — newest first.** `OtherDocumentsTab` merges `docs` (uploaded, sorted by `uploadedAt`) and `contracts` (ad-hoc only, filtered by the parent: `contracts.filter(c => !c.employmentRowId)`) into one list, each row dated by `signingDate` when the contract has one (that's the date the user thinks of it by — see "Ad-hoc documents are row-first" above), else by `generatedAt`. Sorted descending by that timestamp.
  - Each contract row renders through the existing `ContractActionButtons` (generate/view/download/delete/sign), unchanged from the old Adhoc smlouvy card.
- **"Prohlášení poplatníka" moved the other way — from Další dokumenty to the Historie pracovního poměru toolbar**, next to `+ Nástup` (both gated on `employment.manage` OR `documents.view`, since the export doesn't require edit rights). It stays a **stateless generate-and-open** AcroForm export — nothing about it is persisted; only the button's location changed. See [PDF form exports — Dotazník + Prohlášení](#pdf-form-exports--dotazník--prohlášení-2026-06-11) above for the endpoint.

### Tour delta copy

Because both moves relocate a control users already rely on, the tour steps for `emp-doc-tax-declaration` and `emp-doc-generate` (and `emp-contract-sign`, for the upload-menu change below) carry `deltaTitle`/`deltaBody` overrides so a returning user's "Co je nového" mini-tour explains *where it went*, while the full tour and Nápověda page keep neutral first-timer copy. `appTour.version` bumped 14 → 15. See [Onboarding Tour & Nápověda](onboarding-and-help.md#step-definition--tourstep) for the mechanism.
...
⚠️ **That strip is now a permission boundary, not just tidiness (2026-08-03).** The `benefits` sub-doc serves two audiences: Multisport (owned by the editor above, `benefits.edit`) and the HR fields stored beside it — pojišťovna, bankovní účet, home office, zaučování, příplatky — which are part of the employee form and follow `employees.edit`. So `GET /:id/benefits` accepts `benefits.view` **or** `employees.edit` (withholding the `multisport*` keys from the latter), and `PUT /:id/benefits` accepts `employees.edit` **or** `benefits.edit` while stripping `multisport*` **server-side** (`MULTISPORT_FIELDS` in `routes/employees.ts`) — otherwise the wider gate would be a way around the narrower one. `PUT /:id/multisport` still requires `benefits.edit` and is the only writer of those keys.

**Why it changed:** the employee form is one Uložit that fans out to four endpoints behind three permissions. With the benefits section gated on `benefits.edit`, a user holding `employees.create` + `employees.edit` but no benefits permission created the employee, saved contact + documents, then took a 403 from the benefits PUT. The form stayed open showing the backend's raw English "Insufficient permissions", and each retry POSTed **another** employee — three records for one person in production on 2026-07-30. See also the `createdId` ref in `EmployeeFormPage.doSave()`, which makes a retry after a partial save PATCH instead of POST.
...
- They apply to **both active and terminated employees**. This is why they live in `applyDerivedStatus` rather than `computeEffectiveRootFields`. (Historically the latter returned `null` for terminated employees and would have skipped the write for exited staff; since 2026-08-03 it folds their last session instead, but the date columns stay here — `applyDerivedStatus` is the one path that runs on every write *and* the nightly sweep.)
...
⚠️ **"Not terminated" means the full end-date rule, not just an Ukončení row (fixed 2026-08-03).** A session ends via `sessionEndDate()` — Nástup `endDate` → in-effect `délka smlouvy` Dodatek (empty value = doba neurčitá, clears it) → Ukončení row, in that precedence — and `computeEffectiveStatus` uses the *same* helper. They used to disagree: the status walker applied all three rules while this one recognised only the Ukončení row, so a **fixed-term session that simply expired stayed "not terminated" forever** here and could win the most-recent-active contest against genuinely later sessions. Johana Dvořáková's row showed `servírka / DPP / gastro` from a 2023 fixed-term DPP that had no Ukončení row, instead of the `recepční / PPP / recepce` she actually left from in 2026. Keep the helper shared; two copies of this rule is what caused the bug.

**No active session → fall back to the LAST session.** Previously this returned `null`, and the two callers disagreed about what `null` meant: `resyncRootFields` blanked `current*`, while the nightly `refreshEffectiveRootForAllActive` did `continue` and left whatever was there. So a terminated employee's row showed either nothing or a stale value depending on *how* they were terminated (an employment write vs. a date passing). Now the last session is folded either way — an upcoming contract for a before-start employee, and **the job they left from** for a terminated one, which is what the Ukončení tab should show. `null` is returned only when there is no Nástup row at all. `currentContractDuringLeave` / `leaveContractType` are forced off on this fallback: "a second job worked during leave" is a statement about someone currently working.
...
## Vacation-hour ledger (v4.9.0)

A per-employee, per-calendar-year running **balance** of vacation hours — nárok (entitlement), čerpáno (consumed, by month) and zůstatek (remaining). This is **distinct from vacation requests** (`vacationRequests`, the approval workflow documented in [Vacation (Dovolená)](vacation.md)): the ledger is a bookkeeping total fed from payroll, not a request/approval trail. A vacation *request* being approved does not itself write to the ledger — the ledger's "čerpáno" figures come from the payroll period, not from requests (see "Payroll-lock feed" below).

All figures are in **HOURS** — the same unit the payroll engine computes (`vacationHours`) and the AVENSIO payroll-system export uses, so there is no day↔hour conversion anywhere in this feature.

### Data model

`employees/{employeeId}/vacationLedger/{year}` (doc id is the year as a string, e.g. `"2026"`):
...
```ts
{
  ...
  paidOutHours: number | null,       // Proplaceno — termination payout (MAY be negative, see below)
  months: {
    "1".."12": {
      hours: number,
      source: "avensio-seed" | "payroll-lock" | "manual",
      updatedAt: Timestamp,
      updatedBy: string | null,      // uid, or null for the AVENSIO seed script
    }
  },
  updatedAt: Timestamp,
...
- **`"payroll-lock"`** — written automatically when a payroll period is locked (see "Payroll-lock feed" below).
- **`"manual"`** — hand-edited on the employee detail page.

### Service — `functions/src/services/vacationLedger.ts`
...
- `sumConsumed(months)` / `remainingHours(ledger)` — the shared pure math, used identically by the read endpoint and (client-side, reimplemented) by the frontend ledger table.

### Endpoints — `functions/src/routes/employees.ts`

```
GET /api/employees/:id/vacation-ledger?year=YYYY
```
Gated `employees.view.all` OR `employees.view.nonManagement` — the same read gate as the rest of the employee detail record. Returns `null` when the ledger has never been written for that year, otherwise the stored doc plus the two derived fields `consumedHours` and `remainingHours`.
...
- `{ paidOutHours: number | null }` — Proplaceno, the termination payout (may be negative — see v5.9.3 below). Editable in the UI since v5.9.3, for terminated employees (see "Frontend" below).

Only the **monthly** čerpáno figure must be ≥ 0. Since v5.9.3, **all three annual figures** (Loňská, Letošní, Proplaceno) may be negative — `readHours(body[field], true)` for every one of them, not just the entitlement pair. Proplaceno is the payroll system's balancing column (`Nárok − Čerpáno − Proplaceno = Zůstatek`, see `remainingHours()` above), so it goes negative whenever čerpáno overran the entitlement; a real leaver can carry a value like `−111`. Rejecting negative Proplaceno (the behaviour before v5.9.3) made the field unable to hold that data at all.
...
Read-only "Dovolená" section on **Můj profil** (`/muj-profil`), letting an employee see their own vacation-hour spending. Defined in `functions/src/routes/selfService.ts`, mounted on the `/me` router. Gated by the new `vacation.balance.view.self` permission (see below) and, like every other route on this router, resolves the caller's employee id server-side from `users/{uid}.employeeId` — never from the URL — so a caller can only ever see their own ledger.

**Why the admin route (`GET /employees/:id/vacation-ledger`) could not simply be reused:** it is gated on `employees.view.all` / `employees.view.nonManagement`, which a plain employee does not hold — reusing it would 403 for exactly the users the section is for — and it trusts the employee id in the URL path, which conflicts with the self router's standing contract (stated in its own header comment) that the employeeId is always resolved from the auth token via `getCallerEmployeeId(req.uid!)`, never the path.

There is deliberately **no self `PATCH` counterpart** — editing a vacation balance stays exclusively behind `employees.vacationBalance.manage` on the admin route; an employee can view their own ledger but never edit it.

### Payroll-lock feed — `functions/src/routes/payroll.ts`

`PATCH /payroll/periods/:id` (the lock/unlock toggle, gated `payroll.lock`) mirrors vacation hours into the ledger **only on a genuine `unlocked → locked` transition** — a repeated lock (the handler doesn't reject re-locking an already-locked period) does not re-run the feed.
...
- Backend catalogue: `functions/src/auth/permissions.ts`, "Zaměstnanci" group, label *"Upravit zůstatek dovolené (po měsících)"*; granted to the `director` builtin type.
- Frontend catalogue: `frontend/src/lib/permissions/catalog.ts`, under the Zaměstnanci page group → new subsection **"Dovolená – zůstatek"** (a single level-1 item — there's no separate "view" key; reading the ledger piggybacks on the existing `employees.view.*` gate the employee detail page already requires).
- `admin` covers it implicitly via `system.admin`; any other role/type needs the key granted explicitly in Nastavení → Uživatelské typy.

### Permission — `vacation.balance.view.self` (v4.12.1)

Gates the read-only "Dovolená" section on Můj profil, i.e. the self-service mirror endpoint above.

- Backend catalogue: `functions/src/auth/permissions.ts`, group **"Vlastní profil"**, label *"Zobrazit vlastní zůstatek dovolené"*.
- Frontend catalogue: `frontend/src/lib/permissions/catalog.ts`, "Můj profil" page section, level 1 (sibling of `sensitive.reveal.self` and `self.profile.requestEdit`).
- **Granted to nobody by default** — deliberately absent from `BASE_SELF` and held by no builtin type, so the section stays inert on every existing account until an admin grants it per user type in Nastavení → Uživatelské typy. `system.admin` still expands to it like every other key.
- **This is a real server-side gate, not a display-only convenience key** — `selfServiceRouter.get("/employee/vacation-ledger", requirePermission("vacation.balance.view.self"), …)` enforces it on the endpoint itself, the same pattern `sensitive.reveal.self` uses to gate `POST /me/employee/reveal` on the same router. The frontend `can("vacation.balance.view.self")` check on `EmployeeSelfPage.tsx` is only the show/hide affordance; removing the frontend check without also holding the permission would still 403. Contrast this with `contracts.generate.bulk` (v4.12.0, see [Contracts & Templates](contracts.md)), which **is** display-only by necessity (bulk generation is just N client-orchestrated calls to endpoints already gated per document) — `vacation.balance.view.self` is not that kind of key.

### Frontend — `VacationLedgerSection.tsx`

`frontend/src/components/VacationLedgerSection.tsx` is a **shared** component with two mount points:

- The employee detail page's **Detail** tab (`EmployeeDetailPage.tsx`, between Benefity and Historie změn) — a collapsible section, default-collapsed like every other section on that tab, fetching lazily (only once expanded).
- **Můj profil** (`EmployeeSelfPage.tsx`, v4.12.1) — a plain (non-collapsible) "Dovolená" section rendered after the "Pracovní poměr" block, gated on `can("vacation.balance.view.self")`.

- **Year switcher** (`‹ year ›`), opening on the **current year** (v4.12.2). Read through `lib/clock`, not `new Date()`, so the non-prod test clock moves it too.
  - **Ceiling = the current year**: the "next year" arrow disables there. The ledger is fed by payroll locks, so a future year holds nothing to show.
  - **Floor = `MIN_YEAR = 2022`**: how far back the switcher goes, *not* a claim that data exists that far back. (It was `FIRST_YEAR = 2026`, the earliest AVENSIO-seeded year — which also made the section open on 2026 regardless of the date, i.e. on the wrong year from Jan 2027. Lowering it lets earlier years be back-filled later without touching this.)
- **Empty year** (v4.12.2). A year with no ledger doc shows a message instead of a row of dashes. With `canManage` the **grid still renders underneath** ("zadejte je dvojklikem níže") — double-clicking a dash is the ONLY way a year's record is created (the PATCH upserts it), so hiding the grid would make a fresh year impossible to fill, which the current-year ceiling makes reachable every January. Without `canManage` the message stands alone: a reader has nothing to do with dashes.
- **One table, the same column order as `VacationLedgerTable` on `/dovolena`** (2026-08-19): `Loňská · Letošní · Nárok · [1–12] · Čerpáno · (Proplaceno) · Zůstatek`. Loňská/Letošní/Proplaceno are editable via double-click when `canManage` and accept negatives; Nárok (= Loňská + Letošní), Čerpáno and Zůstatek are derived read-only, and Zůstatek renders in a warning colour when negative. **This replaced a summary line above a separate two-row month table** — the same ledger rendered in two different shapes depending on the page, so a figure could not be compared by eye between the employee detail page and the aggregate table, and the split carried no meaning the grid does not. The month block is fenced with a 2px border on both sides (`.groupHead` / `.blockStart`) so `Nárok | měsíce | Čerpáno` still reads as the arithmetic it is.
- **Proplaceno (v5.9.3).** The `paidOutHours` field had always been stored and accepted by the PATCH endpoint, but no UI rendered it — dropped in the v4.9.x ledger redesign — so a termination payout could not be recorded and a leaver kept showing a balance payroll had already settled. Re-added between Čerpáno and Zůstatek so the row reads as the arithmetic it is (`Nárok − Čerpáno − Proplaceno = Zůstatek`); same double-click-to-edit interaction as Loňská/Letošní, gated behind the same `canManage` prop (i.e. `employees.vacationBalance.manage` — no new permission), and an empty input clears it.
  - **New `isTerminated?: boolean` prop** (default `false`) on `VacationLedgerSection`. `EmployeeDetailPage` passes `employee?.status === "terminated"`; `EmployeeSelfPage` takes the default, so Můj profil is unaffected.
  - **Visibility rule: `isTerminated || paidOutHours != null`** — not `isTerminated` alone. Proplaceno only ever gets *filled in* on termination, so the field stays hidden for an active employee to avoid a permanently-empty row — but a value that is already set stays visible even if the employee is later reactivated or the flag is wrong for any reason, because `remainingHours()` on the server subtracts it from Zůstatek **unconditionally**, regardless of employment status (see the PATCH note above). A balance quietly shrunk by a figure the UI hides would read as a bug, not as a feature.
- **Month cells 1–12** are double-click-to-edit when `canManage`; a cell whose `source === "manual"` is visually flagged (background + `*`) using the **same "manual override" convention as Payroll's on-screen entry rows** — a deliberate reuse of an existing visual language rather than inventing a new one. A month with no record at all renders dimmed, so "nothing recorded" stays distinguishable from "recorded as zero".
  - The old layout's separate **CELKEM** column is gone: it was Čerpáno under another name, and in one row the two sat side by side.
  - **No lock glyphs on the month headers**, unlike `VacationLedgerTable`. Lock state comes from payroll periods, which only `GET /vacation/ledger-overview` returns; the per-employee `readLedger` payload has no `periods`. Adding them here would mean a second query per employee-detail page load.
  - Figures render through the shared `formatLedgerValue` (plain number, Czech decimal comma). The `" h"` suffix that used to ride on each annual figure would have meant mixed formatting across seventeen columns, so **the unit is stated once in a legend under the grid**, as on `/dovolena`.
...
- **`canManage: boolean` prop.** `EmployeeDetailPage` passes `can("employees.vacationBalance.manage")`; **Můj profil always passes `canManage={false}`** (view-only for every user, since there is no self `PATCH`). Every editing affordance is gated behind this one prop — `startEdit()` hard-returns when `!canManage`, the double-click handlers/`.editable` CSS class/"Dvojklik pro úpravu" tooltip are all conditional on it, and the manual-override tooltip degrades from `"Ručně upraveno · dvojklik upraví"` to plain `"Ručně upraveno"`. This is what let the section be reused on Můj profil in v4.12.1 with **no component changes** beyond the prop rename below.
- **`basePath: string` prop (v4.12.1, replaces the old `employeeId` prop).** The ledger endpoint *without* the year — `EmployeeDetailPage` passes `/employees/{id}/vacation-ledger`, `EmployeeSelfPage` passes `/me/employee/vacation-ledger`. The component appends `?year=` for GET and `/{year}` for PATCH; since the PATCH path is only ever reached when `canManage` is true, it is unreachable from the self `basePath` in practice (there is no PATCH route mounted under `/me` to call anyway).
