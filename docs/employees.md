...
- **Summary line**: Loňská + Letošní (each editable via double-click when `canManage`) / Nárok (= Loňská + Letošní, derived read-only) / Čerpáno (derived, read-only) / **Proplaceno** (v5.9.3, conditionally shown — see below) / Zůstatek (derived, read-only, rendered in a warning colour when negative). Loňská/Letošní/Proplaceno accept negative values; the monthly cells do not. No CSS changes were needed to fit the extra item — `.summary` was already `flex` + `flex-wrap: wrap`.
