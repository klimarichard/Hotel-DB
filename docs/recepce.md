...
### Handover warnings ("Předávací protokol" tab)

Two warning `type`s share the `handoverWarnings` collection, both generated by the
sign/revert flow (`resyncChainAfter`) and never by content edits. Each is a `.set()`
...
- **`type: "chain"` — Nenavazující předání.** `syncChainWarning()` compares this
  ...
  are treated as `"chain"` by the client.
- **`type: "late"` — Pozdní příchod.** `syncLateWarning()` flags a **Převzal** that
  landed after the next shift's start — **19:00 for a night (noc) protocol, 07:00
  for a day (den) protocol**, i.e. a den handover due 19:00 the same day and a noc
  handover due 07:00 the next morning. Compared in Europe/Prague wall-clock at
  minute precision (`evaluatePrevzalLateness`, formatToParts + zero-padded
  `YYYYMMDDHHMM` string compare — DST-safe, no manual offset math). The **Předal
  stamp is irrelevant.** Upserted at `handoverWarnings/{hotel}_{id}_late` (a
  separate doc, so a protocol can carry both warnings with independent read state)
  with `actorUid/actorName` (the late Převzal), `prevzalLabel` (Prague-formatted
  sign time) and `cutoffLabel` ("07:00"/"19:00").

```
handoverWarnings/{hotel}_{id}[_late] = {
  hotel, handoverId, type: "chain" | "late", shiftDate, shiftType,
  actorUid, actorName,
  expectedUid, expectedName,   // chain only
  prevzalAt, prevzalLabel, cutoffLabel,  // late only
  createdAt, read, readAt, readBy
}
```

Surfaced on the **Upozornění → Předávací protokol** tab (renamed from "Nenavazující
předání") — `frontend/src/pages/upozorneni/HandoverWarningsTab.tsx`: two unread
sections (Nenavazující předání / Pozdní příchody, type-specific columns) plus a
shared **Přečtené** section. `GET/POST /api/handover-warnings` +
...
existing **`changeRequests.review`** permission (no new key). The **combined unread
count** (both types) feeds the tab count and the sidebar `/upozorneni` badge via
`HandoverWarningsContext` (`GET /handover-warnings/unread-count` — a `.count()` of
`read==false`), refreshed on nav + the 60 s loop.
