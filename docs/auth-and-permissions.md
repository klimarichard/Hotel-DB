...
Gates the `/navody` guides page (uploaded PDF tutorials + external links, grouped into categories). `nav.guides.view` covers both the page/menu and every read endpoint; `guides.manage` covers every write (categories + guides, including reorder). See [Guides (Návody)](guides.md) for the full feature writeup.
