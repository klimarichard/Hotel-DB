...
- **Narrow, purpose-built projections beat widening a view permission.** `GET /api/employees/plan-options` (v5.11.1) exists because the shift-plan employee picker needed *names* while its callers (FOM / `manager`) hold no `employees.view.*` key at all. Rather than grant them record-level read, the endpoint returns `id`/`firstName`/`lastName`/`displayName` only and is gated on the permission the caller already needs to act on the result — `shifts.planEmployees.manage` OR either `employees.view.*` key. It still applies the `isNonManagementScoped` filter above. Prefer this shape over adding a broad view permission to a type that needs one field.
- `getManagementEmployeeIds` resolves each user's type via `roleTypeFromUserDoc(u)` (see "Legacy `role` fallback for reading OTHER users" below) against the per-type `management` flag.
...
### Legacy `role` fallback for reading OTHER users (v4.12.0)

The retirement above only covers **the current caller's own** resolution path — `requireAuth` reads the JWT custom claim, which was backfilled onto every account and always carries `roleType`, so a legacy account works perfectly when *it* logs in. Code that inspects **someone else's** `users/{uid}` Firestore doc has no claim to fall back on, and some docs written **before** the RBAC overhaul were never rewritten with a `roleType` field — they still carry only the older `role`. Reading `u.roleType` alone on such a doc resolves to `undefined` → an empty permission set → that user silently drops out of whatever set is being built.

This bit `getManagementEmployeeIds` specifically: a **management**-type user whose doc was one of the legacy ones was *not* added to the hidden-records set — the opposite of the intended default-deny, since their employee record then stayed **visible** to non-management viewers instead of hidden. Measured before shipping the fix: **0** such docs on production (the RBAC-overhaul migration predates prod going live, so the condition was never reachable there) but **10** on staging.

Fixed in code rather than by a one-off backfill migration, because the code must tolerate a legacy doc regardless of whether one exists today: **`roleTypeFromUserDoc(u: { roleType?: unknown; role?: unknown })`** (`functions/src/auth/permissions.ts`) returns `u.roleType` if it's a non-empty string, else `u.role`, else `undefined`.

**Standing rule:** resolve **another** user's type through `roleTypeFromUserDoc()`, never `u.roleType` directly; for the **current** caller, prefer the already-resolved `req.roleType` claim. Readers updated to the helper as of v4.12.0: `userIsAdmin`, `GET /auth/me` (prefers `req.roleType`, falling back through both doc generations only if the claim is somehow absent), `GET /auth/logout-authorizers`, `GET /handovers/:hotel/revokers`, `getManagementEmployeeIds`, and the `roleTypes` delete-guard (blocks deleting a type still assigned to someone).
...
| `POST /api/users/trigger-scheduled-deactivations` | Manually run the scheduled-deactivation sweep (v3.7.0) | `system.triggers`; mirrors the `checkScheduledDeactivations` job, writes a `manual-trigger` audit entry. Wired into Settings → Úlohy as of **v5.11.6** |
| `POST /api/exchange/trigger-snapshot-sweep` | Manually sweep směnárna snapshots older than 6 months (v5.11.6) | `system.triggers`; mirrors `sweepSmenarnaSnapshots`, writes a `manual-trigger` audit entry |
| `POST /api/payroll/trigger-refresh` | Manually re-fold every published plan into its payroll period (v5.11.6) | `system.triggers`; mirrors `refreshPayroll` via the shared `refreshAllPublishedPayrollPeriods()`, writes a `manual-trigger` audit entry. Locked periods and manual overrides are untouched |
...
**Employee-link picker scope — active *and* `before-start` (v5.11.3).** Both employee dropdowns in Settings → Uživatelé (the *Přidat uživatele* form and the *Propojit se zaměstnancem* modal) are fed by one `employees` array in `SettingsPage.tsx`, and it originally came from a single `GET /api/employees?status=active`. That silently excluded **future hires**: `computeEffectiveStatus()` (`routes/employees.ts`) treats a session as started only once `startDate <= today`, so someone with a future-dated Nástup is stamped `"before-start"`, not `"active"` — yet that is exactly the group whose account is normally created *before* day one, so linking them was impossible with no error shown anywhere.

The picker now fetches **`status=active` and `status=before-start` in parallel** and merges the two lists, stamping each row with the status it came from so future hires can render as `Příjmení Jméno (před nástupem)`. Terminated employees stay excluded. `GET /api/employees` supports only a **single** `where("status", "==", …)` equality — there is no `in` — so this is a client-side fan-out, the same pattern `EmployeesPage.tsx` and `AuditLogPage.tsx` already use (three parallel calls each); no backend change was needed. Each call carries its own `.catch(() => [])` so one failing query degrades the list rather than blanking it.

> ⚠️ **Treat a lone `?status=active` fetch — or any status set that is not all three — as suspect** whenever a feature concerns people who are hired but have not started. `upozorneni/PendingShiftChangeRequestsTab.tsx` and `PendingShiftOverridesTab.tsx` had the same gap in their **name-resolution maps** (they loaded active + terminated only, so a `before-start` employee's row fell through to the raw `dtb_…` doc id and lost its profile link); both now fetch all three statuses. That was reachable because `GET /employees/plan-options` staffs shift plans from the **entire** roster with no status filter, and `POST /shifts/plans/:planId/shiftOverrides` takes `employeeId` from the body without checking lifecycle status — so a planner can raise an override against someone who starts next month.
>
> When adding a status list to a `Promise.all`, widen the **destructure** in the same edit: the calls resolve positionally and every list is `EmployeeMini[]`, so inserting one without renaming the bindings silently binds the wrong array and TypeScript reports nothing.
