...
> ⚠️ **Treat a lone `?status=active` fetch as suspect** whenever a feature concerns people who are hired but have not started. Known remaining cases (name-resolution maps only, not pickers): `upozorneni/PendingShiftChangeRequestsTab.tsx` and `PendingShiftOverridesTab.tsx` load active + terminated, so a `before-start` employee's name will not resolve there.
