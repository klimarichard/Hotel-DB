...
- Editor: a "Může být prázdná" checkbox column in the "Vlastní proměnné" modal (`ContractTemplatesPage.tsx`). Generation: `GenerateContractModal.tsx` appends a muted "(nepovinné)" after the slot's label, since otherwise the only way to discover a slot is optional is to try generating.
