...
**Layout (v4.8.2).** The modal was widened (`min(880px, 96vw)`) and each slot's name/type/default value now sit in a **single table row** — previously the default-value control lived in a second, indented row underneath the name/type/(then-present Odstavec) row. Every field for a slot is edited in one place now.
...
`fullName` was also removed — no template referenced it, and it's trivially `{{firstName}} {{lastName}}` when needed. The surviving canonical booleans are `isCzech`, `isMale`, `isHalfTime` (v4.8.2, below), `hasPermanentResidence`, `hasProbation`, `hasEndDate`, plus the `isDodatek*` family.
