...
**Generation UI — `GenerateContractModal.tsx`.** When the loaded template has custom slots (`usedCustomVars(template)`), a "Vlastní proměnné" input table renders above the existing "Hodnoty proměnných" overview: a checkbox for `bool`, a native `<input type="date">` for `date`, `type="number"` for `number`, plain text otherwise. This is a **generation-time-only guard, stricter than built-in variables**: an empty built-in variable only adds a "Chybějící údaje" warning and generation proceeds anyway, but an empty custom slot is a blank in the contract's own sentence (a penalty amount, a deadline) — the "Generovat PDF" button stays `disabled` while `missingCustomVars(...).length > 0`.
