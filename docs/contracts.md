...
- `{{hoursPerWeek}}` (v3.5.0; defaulted since v5.6.1) — contracted hours per week. A stored `hoursPerWeek` on the effective employment row always wins; when there is none, `hoursPerWeekValue()` falls back by contract type: **HPP → 40**, **PPP → 20**, DPP → blank. The picker label was **"Počet hodin týdně (PPP)"** until v5.6.1 and is now just "Počet hodin týdně" — the variable is no longer PPP-specific.
  - The fallback exists because the employee form persists `hoursPerWeek` **only** for PPP rows and writes `null` for HPP (`EmployeeDetailPage.tsx`, the Nástup payload), so before v5.6.1 the variable rendered blank on every HPP contract — and, since no production row had ever been given a value, on PPP ones too.
...
- `CUSTOM_VAR_KEYS` — `["var1", …, "var10"]`, `isCustomVarKey(key)` checks membership. ⚠️ **Superseded (v5.2.0).** `CUSTOM_VAR_KEYS` is now the *engine's* 25-slot recognition ceiling, shared with Dokumenty; `CONTRACT_VAR_COUNT = 10` is what this page still offers and the server still accepts. See ["Custom-variable engine widened"](#custom-variable-engine-widened--longtext-math-case-and-25-slot-parity-with-dokumenty-v520) below and the dedicated [custom-variable-engine.md](custom-variable-engine.md).
- `CustomVarType` — `"text" | "date" | "number" | "bool"` at the time this section was written. Grew to `"list"` (undocumented gap — see the v5.2.0 section below) and then `"condition"` (v4.10.0, next section), `"longtext"` / `"math"` / `"image"` (v5.2.0), and `"time"` (Čas — an `<input type="time">` value printed verbatim, comparing as text). `longtext` also gained optional per-slot `fontSize` / `lineHeight` typography. See [custom-variable-engine.md](custom-variable-engine.md) for the full current shape.
...
- **Preview** — `buildPreview()` (`frontend/src/lib/templatePreview.ts`; named `buildPreviewVars` at the time this section was written) now prefers a custom slot's configured default over generic mock text: a `literal` default is formatted and shown as typed, a `fixedVar` default resolves against the same `MOCK_TEXT` sample values the rest of the preview already uses. Falls back to `MOCK_CUSTOM_RAW_BY_TYPE` sample text only when the slot has no default — a blank slot would otherwise collapse the line and misrepresent the layout being checked.
...
**Backend validation — `functions/src/routes/contractTemplates.ts`.** `PUT /:id` accepts an optional `variableDefs` field and rejects the write (400) via `isValidVariableDefs` when it isn't a plain object whose entries are all: a known key (`var1`..`var10` only — the ten-slot ceiling this route enforces, separate from the 25-slot engine ceiling, see the v5.2.0 section below), an object with a `type` in the full engine union (`text | longtext | date | number | bool | list | image | condition | math`; `condition` arrived in v4.10.0, `longtext`/`math` in v5.2.0, `image` shipped just after v5.2.0), a `label` string ≤ 60 chars, a valid optional `default` (v4.8.2 — see "Default values for custom variables" above), a valid optional `condition` (v4.10.0 — see "User-defined derived conditional variables" below), a valid optional `options` (`list`), a valid optional `images` (`image` — see "A ninth custom-variable type" below), and a valid optional `formula`/`decimals` (`math`, v5.2.0). Omitting `variableDefs` from a `PUT` leaves the stored config untouched (`ref.set(payload, { merge: true })` only writes the field when present) — older templates saved before this feature are unaffected.
...
## Custom-variable engine widened — longtext, math, `{{#case}}`, and 25-slot parity with Dokumenty (v5.2.0)

The custom-variable engine (`frontend/src/lib/contractVariables.ts`) grew substantially in v5.2.0, driven by Dokumenty needing the same mechanics this page already had (formulas, conditions) plus two things neither page had yet (a long-text type, a value-based switch). Because the engine is genuinely shared, the mechanics themselves are now documented once, in **[custom-variable-engine.md](custom-variable-engine.md)** — this section covers only what changed on **this** page and where it now differs from Dokumenty.

**New types.** `CustomVarType` gained `"longtext"` (HTML-escaped free text, `\n` → `<br>` — the only type that escapes; plain `"text"` stays raw so production templates that already pass small HTML fragments through it keep rendering identically) and `"math"` (`CustomVarDef.formula` + `CustomVarDef.decimals`, evaluated by a hand-written formula parser — never `eval`; see the engine doc for the grammar and why). Both render in the "Vlastní proměnné" table exactly like every other type: one row, the same label/optional columns, a type-specific control in the last column (`renderFormulaEditor` for `math`; a `<textarea>` for `longtext`, alongside the existing default-source picker).

⚠️ **Also newly documented here: the `"list"` / "Seznam" type**, which in fact shipped earlier (alongside Dokumenty) but was never written up on this page — a documentation gap, not a recent change. `CustomVarDef.options?: string[]` — up to 30 choices, ≤100 chars each, the picked value substituted verbatim (no code/label split). A list slot's default must be one of its own choices, so "Z proměnné" is not offered as its default source. See the engine doc for the full write-up; it applies to this page unchanged.

**`math` formulas may reference built-ins, unlike on Dokumenty.** `formulaDependencies` is not filtered to custom slots, and `resolveComputedVars` is called here with `resolveComparableRaw(effectiveEmployeeData)` as its `fixedRaw` argument (`GenerateContractModal.tsx`) — so a contract's `math` slot can write `salary * 0,15` and have `salary` resolve against the bound employee, something Dokumenty cannot do (it binds no employee, so an identifier that isn't a custom slot there resolves to nothing and blanks the whole formula). One consequence: this page's `customVarWarning()` cannot run Dokumenty's "unknown operand in formula" check — an unrecognised identifier here might legitimately be a valid built-in, so only an **empty** formula is flagged (`emptyFormulas`), not one that merely fails to evaluate against a dummy value.

**Condition operands widened to include custom slots.** Until v5.2.0, a `condition` slot's `leftKey`/`right` could only name a `COMPARABLE_VARS` built-in. The config UI's operand `<select>`s now offer a second `<optgroup>` of the template's *other* custom slots (`comparableTypeOfCustom`, narrowed to the left operand's raw type; a slot comparing itself, or a `condition` slot, is excluded — the latter because `comparableTypeOfCustom` returns `null` for `"condition"`, closing off a cycle at the type level rather than by runtime detection). Built-in-only conditions are untouched: a stored condition over `COMPARABLE_VARS` resolves, renders and evaluates exactly as before. See "Editable condition operands" above for the matching preview-panel change.

**The `{{#case var1 = Praha}}…{{/case}}` switch.** New "⇄ Přepínač…" button in the variable panel opens a small dialog that assembles a `{{#case}}`/`{{/case}}` pair around the cursor without hand-typing the compared value — for a `"list"`-typed slot it offers the slot's own configured choices as a dropdown instead of a free-text box, which is the point: a switch matches by **retyped text**, so the one way it silently fails is a value that differs from the configured choice by a character. Matching itself is on **raw** typed values (a number's raw form has no thousands separator, entities are decoded, whitespace/case are normalised) — see the engine doc for the full matching rule and the `fillTemplate` third-argument contract.

**Slot count stays at `CONTRACT_VAR_COUNT = 10`, deliberately, while Dokumenty moved to 25.** A contract is a signed legal document; widening its slot space buys nothing there. Recognition (`isCustomVarKey`) is still shared at the engine's 25-slot ceiling, so `{{var15}}` typed or pasted into a contract template surfaces as an unnamed/"Mimo rozsah" slot in the editor rather than printing literal braces into a signed PDF — but the server still refuses to persist a `variableDefs` entry past `var10` (`CUSTOM_VAR_KEYS` in `functions/src/routes/contractTemplates.ts` is a 10-key set, distinct from the 25-key set in `functions/src/routes/dokumenty.ts`), so the picker cannot offer, and the config UI cannot save, what it cannot save server-side. `customVarWarning()` on this page reports any such out-of-range slot explicitly ("Mimo rozsah: … – šablony smluv nabízejí jen 10 vlastních proměnných") rather than silently omitting it from the table.

**What's still different from Dokumenty, and why**, now that the type set is identical on both pages: the config dialogs remain two separate implementations (`ContractTemplatesPage.tsx` / `DokumentyPage.tsx`), because the two axes that actually differ — the condition-operand catalogue (built-ins available here, not there) and the default-value source (`fixedVar` available here, not there) — both trace back to the same fact, that this page binds a real employee/company record and Dokumenty binds none. See [custom-variable-engine.md](custom-variable-engine.md#what-differs-per-page) for the full comparison table.

---

## A ninth custom-variable type: `image` ("Obrázek")

Shipped on top of the v5.2.0 widening above: `CustomVarType` gained a **ninth** value, `"image"` — a `"list"` whose choices each carry a picture (`CustomVarDef.images?: CustomVarImageOption[]`, up to `CUSTOM_VAR_MAX_IMAGES = 8` choices, each ≤`CUSTOM_VAR_IMAGE_MAX_CHARS = 120_000` characters of base64, ~90 KB). Picking a choice at generation substitutes that choice's `<img>` into the document — one variable instead of one `{{#case}}` branch per picture, for the common case where only the picture changes per answer. `{{#case}}` (see above) remains the right tool when more than the picture varies per branch.

The full mechanics — why the picture must be inlined as base64 rather than a Firebase Storage URL (`pdfRenderer.ts`'s SSRF guard), why `isImageDataUri` is a security boundary rather than a format check (and why it refuses SVG despite SVG being an image), why the raw value of an `image` slot is the chosen choice's **label** (so it composes with `{{#case}}`/`condition` with no second variable needed), the `missingCustomVars` asymmetry against `list` (an empty `image` slot is exempt from blocking; an empty `list` stays required and degrades to free text), the interaction with the `PUT /:id` size guard, and a pagination landmine in `pdfRenderer.ts` — are documented once, shared with Dokumenty, in **[custom-variable-engine.md](custom-variable-engine.md#image-slots-obrázek)**. This section covers only what's specific to Šablony smluv.

**Editor (`ContractTemplatesPage.tsx`).** The images editor for an `image` slot is shaped like the `"list"` options editor — one row per choice, in author order — with a per-row file picker in place of a plain text input; one hidden `<input type="file">` serves every choice's upload button. `frontend/src/lib/imageDownscale.ts`'s `prepareImageDataUri` reads and, if needed, downscales the picked file before it's stored. The editor also tracks a **running total** across every `image` slot's pictures on the template — including slots whose placeholder is no longer used in the text ("orphaned"), since their pictures are still stored on the same document and still count toward its 1 MiB ceiling — and shows a warning once that total passes `IMAGE_TOTAL_WARN_CHARS = 600_000` characters (~60 % of the ceiling, chosen to leave headroom for the rest of the template's HTML and any pictures pasted directly into the document).

**Fill-in (`GenerateContractModal.tsx` / `BulkGenerateModal.tsx`).** An `image` slot renders as a `<select>` over the configured choice labels, with a live thumbnail preview of the picked choice underneath — two similar-looking pictures are otherwise indistinguishable until seen, and the PDF opens in a new tab with nothing stored, so this is the last chance to notice a wrong pick. A slot with no usable choice (label **and** picture both present) shows a plain notice instead of an unfillable dropdown and prints nothing for that slot, matching the exemption in `missingCustomVars`. `BulkGenerateModal.tsx` (client-orchestrated "Hromadné generování", one document per employee — see the "Hromadné generování" section further down) resolves an `image` slot's chosen picture the same way the single-document modals do.

**Backend (`functions/src/routes/contractTemplates.ts`).** `isValidVariableDefs` gates a slot's optional `images` array via `isValidCustomImages`: at most 8 entries, each an object with **only** `label`/`src`/`width?`/`align?` (an unknown key rejects the whole def, rather than being silently dropped), `label` ≤`CUSTOM_VAR_OPTION_MAX` chars, `src` re-validated against the server's own `isImageDataUri` (the client's copy is a convenience only — this is the check that actually holds), and `width`/`align` restricted to the same closed preset sets the editor offers. `CUSTOM_VAR_MAX_IMAGES`/`CUSTOM_VAR_IMAGE_MAX_CHARS`/`CUSTOM_VAR_IMAGE_WIDTHS`/`CUSTOM_VAR_IMAGE_ALIGNS` are hand-mirrored from `frontend/src/lib/contractVariables.ts`, same lockstep requirement as every other engine constant — see the engine doc.
...
**In-editor preview.** Clicking 👁 swaps the `EditorContent` inside `.a4Page` for a read-only `dangerouslySetInnerHTML` render of `fillTemplate(html, buildPreview(html, variableDefs, previewBools, previewRaw).vars, ...raw)` (`buildPreview`, `templatePreview.ts` — named `buildPreviewVars` at the time this section was written) — the *same* `.a4Page` box, the *same* margins (`margins.top/bottom/left/right` inline styles), the *same* ProseMirror-authored inline typography, so the preview lays out exactly like the editor minus the markers. The dashed ↧ page-break divider (`.a4Page [data-page-break] { border-top: 2px dashed #999; margin: 1cm 0 }`, editor-only styling that never reaches the Puppeteer renderer) is suppressed in preview via `.previewContent [data-page-break] { border-top: none; margin: 0 }` — it's an editor affordance that prints nothing, and leaving its dashed line and 2 cm margin in would misrepresent where content actually breaks across pages.
...
- `buildPreview(html, defs, bools, rawOverrides)` builds the full `{ vars, raw }` pair the preview fills with: `MOCK_TEXT` supplies realistic, roughly realistically-**long** Czech sample values for every non-boolean permanent variable (e.g. `firstName: "Jana"`, `address: "Vinohradská 1511/230, 100 00 Praha 10"`, `salary: "35.000"`) — a blank `{{fullName}}`-style variable would collapse to nothing, making the line shorter than it will ever really be and any tab-alignment check against it a lie. Conditionals resolve to `"ano"`/`""` from the checkbox state; custom slots get a per-type sample from `MOCK_CUSTOM_RAW_BY_TYPE` (a slot the template uses but never configured falls back to `text` sample data; `longtext`'s sample is deliberately three real paragraphs, since a one-line stand-in would misrepresent the vertical space a prose slot actually consumes); `math`/`condition` slots are genuinely **computed** via `resolveComputedVars` rather than sampled — a total is the one value an author cannot eyeball, so faking it with mock text would be worse than not previewing it — see "Custom-variable engine widened" below. `raw` layers ISO/typed values over the printed ones so a `{{#case}}` switch on a permanent variable with no raw form (`{{#case firstName = Jana}}`) still matches, exactly as calling `fillTemplate` with no raw map at all would. ⚠️ **Fixed in v4.10.0:** the `salary` sample used to read `"35 000 Kč"`, which double-counted the currency — `formatSalaryCZ` only ever emits the bare number (dot thousands-separator: `"35.000"`), the "Kč"/",- Kč" suffix is template text, so the old sample silently misrepresented how long the rendered line actually is.
