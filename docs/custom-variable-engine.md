...
The engine's shape (`CustomVarType`, `CUSTOM_VAR_FORMULA_MAX`, `CUSTOM_VAR_DECIMALS_MAX`, `CUSTOM_VAR_MAX_OPTIONS`, `CUSTOM_VAR_MAX_IMAGES`, `CUSTOM_VAR_IMAGE_MAX_CHARS`, `CUSTOM_VAR_IMAGE_WIDTHS`, `CUSTOM_VAR_IMAGE_ALIGNS`, `CUSTOM_VAR_LONGTEXT_FONT_SIZES`, `CUSTOM_VAR_LONGTEXT_LINE_HEIGHTS`, …) is hand-mirrored in **three** places:

1. `frontend/src/lib/contractVariables.ts` — the source of truth for the grammar and every UI-facing rule.
2. `functions/src/routes/dokumenty.ts` — `isValidVariableDefs` / `isValidCondition` / `isValidCustomFormula` / `isValidCustomOptions` / `isValidCustomDecimals` / `isValidCustomImages` / `isImageDataUri` / `isValidCustomFontSize` / `isValidCustomLineHeight`, keyed to a 25-slot set.
