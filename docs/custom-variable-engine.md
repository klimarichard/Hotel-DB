...
| `longtext` | Dlouhý text | yes (textarea) | The **only** slot type whose value is HTML-escaped. `\n` → `<br>` so multi-paragraph prose doesn't collapse to one run-on line inside the `<p>` the placeholder sits in (real `</p><p>` would nest block elements and lose the surrounding paragraph's styling). A blank line therefore prints as two `<br>` — a visible paragraph gap. May carry optional **`fontSize` / `lineHeight`** (see "Longtext typography" below). |
| `date` | Datum | yes (`<input type="date">`) | `formatDateCZ` splits the ISO string rather than parsing a `Date` — no UTC-offset day-shift. |
| `time` | Čas | yes (`<input type="time">`) | The raw `"HH:MM"` (24-hour) value, printed **verbatim** — already unambiguous, and matches what was entered. Needs no per-slot config beyond being an accepted type. Compares as **text** (`comparableTypeOfCustom` → `"text"`): the raw string happens to order lexicographically, but the condition engine only coerces date/number/text, and a time is neither an ISO date nor a number, so it gets equality/emptiness only. |
| `number` | Číslo | yes | `Intl.NumberFormat("cs-CZ")` — Czech thousands grouping, same convention as every other numeric value in the app. Optional **`thousandsSeparator`** (`"space"` \| `"none"`, absent = `"space"`) toggles the grouping per slot: `"none"` prints the digits ungrouped (`1500`) while keeping the Czech decimal comma — it maps straight to `Intl.NumberFormat`'s `useGrouping`. Allowlisted server-side like every other stored enum. |
...
`CUSTOM_VAR_TYPE_LABELS` (`contractVariables.ts`) is the canonical label map; both pages' type `<select>` iterate its keys, so a page's editor never lists a type the engine doesn't know about (in practice both pages currently offer all ten — see the per-page table at the bottom).
...
A `longtext` slot may carry `CustomVarDef.fontSize?: string` (e.g. `"12pt"`) and `CustomVarDef.lineHeight?: string` (e.g. `"1.5"`), both **optional** and both chosen when the variable is **configured** — not at generation, where the slot is filled through a plain textarea with no formatting UI. Absent means inherit the template's own text styling, so every existing `longtext` slot is unaffected. They exist because a block of prose dropped into a document often wants to read a size smaller and tighter (or larger) than the surrounding template, and there was previously nowhere to say so.

`formatCustomValue("longtext", value, def)` applies them by wrapping the escaped, `<br>`-joined body in an inline-styled `<span>` — **a `<span>`, not a block wrapper**, for the same reason the type emits `<br>` rather than `</p><p>`: the placeholder sits *inside* the template's `<p>`, and a block element there would nest block-in-inline and lose the paragraph's styling. `line-height` on an inline span still governs the line boxes its wrapped text generates, so increasing it visibly opens the prose up (it cannot shrink below the containing paragraph's own strut, which is the natural and expected limit).

**Both values are attacker-reachable CSS and are gated against a closed set, exactly like the image width/align presets.** They are echoed into a `style="…"` attribute that Puppeteer renders, so they are **not** free strings: `CUSTOM_VAR_LONGTEXT_FONT_SIZES` / `CUSTOM_VAR_LONGTEXT_LINE_HEIGHTS` (shared constants, mirroring the editor toolbar's own size/spacing selects) are the only accepted values. `formatCustomValue` re-checks membership before emitting the span (belt-and-braces), and — the gate that actually holds — both server validators (`isValidCustomFontSize` / `isValidCustomLineHeight`) refuse anything outside the sets, so a hand-crafted request cannot inject markup through these fields.
...
The engine's shape (`CustomVarType`, `CUSTOM_VAR_FORMULA_MAX`, `CUSTOM_VAR_DECIMALS_MAX`, `CUSTOM_VAR_MAX_OPTIONS`, `CUSTOM_VAR_MAX_IMAGES`, `CUSTOM_VAR_IMAGE_MAX_CHARS`, `CUSTOM_VAR_IMAGE_WIDTHS`, `CUSTOM_VAR_IMAGE_ALIGNS`, `CUSTOM_VAR_LONGTEXT_FONT_SIZES`, `CUSTOM_VAR_LONGTEXT_LINE_HEIGHTS`, `CUSTOM_VAR_THOUSANDS_SEPARATORS`, …) is hand-mirrored in **three** places:

1. `frontend/src/lib/contractVariables.ts` — the source of truth for the grammar and every UI-facing rule.
2. `functions/src/routes/dokumenty.ts` — `isValidVariableDefs` / `isValidCondition` / `isValidCustomFormula` / `isValidCustomOptions` / `isValidCustomDecimals` / `isValidCustomImages` / `isImageDataUri` / `isValidCustomFontSize` / `isValidCustomLineHeight` / `isValidThousandsSeparator`, keyed to a 25-slot set.
