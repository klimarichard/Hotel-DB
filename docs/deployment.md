...
- **Collection-group single-field exemptions** — collection-group queries on a single field need an explicit `COLLECTION_GROUP` index. `firestore.indexes.json` declares one for `benefits.multisport` so the daily `sweepMultisport` cron (and its manual trigger) can find every employee whose Multisport flag is still on, and one for `history.at` so the daily `sweepRecepceHistory` cron (and its manual trigger) can find aged-out Recepce protocol history entries across every hotel via `collectionGroup("history").where("at","<=",cutoff)`.
