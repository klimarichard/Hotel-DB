...
- **Collection-group single-field exemptions** — collection-group queries on a single field need an explicit `COLLECTION_GROUP` index. `firestore.indexes.json` declares one for `benefits.multisport` so the daily `sweepMultisport` cron (and its manual trigger) can find every employee whose Multisport flag is still on, one for `history.at` so the daily `sweepRecepceHistory` cron (and its manual trigger) can find aged-out Recepce protocol history entries across every hotel via `collectionGroup("history").where("at","<=",cutoff)`, and one for `shiftSplits.date` (feature/shift-hour-splits) so the internal Recepce summary ("4D") can range-query manually assigned hour splits across every plan via `collectionGroup("shiftSplits").where("date",">=",from).where("date","<=",to)` — see [Shifts — Manual hour splits](shifts.md). Missing this one specifically is easy to overlook in review: the page works fine until a split doc actually exists inside the queried date range, then 500s.
- **Recepce collections need no NEW composite indexes.** `hotels/{hotel}/shiftHandovers`, `walkins`, and `taxiRides` are all queried with a single-field range **on the same field they're ordered by** (`shiftDate`/`date` `>=`/`<=` + `orderBy` on that same field) — Firestore serves that without a composite index. The retention sweep's `auditLog` queries (`where("collection","==",X).where("timestamp","<=",cutoff)`) reuse the pre-existing `(collection, timestamp desc)` composite index.
...
As of **v5.11.6** every one of the ten scheduled jobs has a matching `POST /api/.../trigger-*` endpoint, and every one is wired into the Settings → Úlohy UI. They exist so an admin can re-run a job after a missed or failed scheduled execution (and to backfill data after a fix). **When you add a scheduled function, add its trigger endpoint here, its `triggerEndpoint` in `services/jobRuns.ts`, AND its entry in `JobsTab.tsx` — those are three separate lists.**
...
| `POST /api/users/trigger-scheduled-deactivations` | `runScheduledDeactivations()` (mirrors the 5-minute `checkScheduledDeactivations` sweep) |
| `POST /api/recepce/trigger-retention-sweep` | `sweepRecepceRetention()` — mirrors the daily `sweepRecepceHistory` sweep; see [Recepce — Retention sweep](recepce.md#retention-sweep) |
| `POST /api/exchange/trigger-snapshot-sweep` | `sweepSmenarnaSnapshots()` — mirrors the daily směnárna snapshot sweep (v5.11.6). Deletes snapshots older than 6 months; nothing references a snapshot, so no business data is lost |
| `POST /api/payroll/trigger-refresh` | `refreshAllPublishedPayrollPeriods()` — mirrors the daily `refreshPayroll` job (v5.11.6). ⚠️ Folds only **published** plans; **locked** periods are untouched and manual overrides / `sickLeaveHours` / notes are preserved (no `discardOverrides`), so only auto-computed cells recompute |

All are gated `requireAuth` + `requirePermission("system.triggers")` and write a `manual-trigger` audit entry per successful call (`extra.trigger` names the underlying job, `extra.result` carries the job's return value). The audit-log write happens *after* the job, so a failed re-run leaves no entry — the failure surfaces in `firebase functions:log`. As of v5.11.6 **all ten** are wired into the **Settings → Úlohy** in-app UI below; none is curl-only.

**Manual runs record job health too (v5.11.7).** Every trigger body runs through `runManualTrigger()` → `runJob(jobId, …)`, so a button press writes `jobRuns/{jobId}` exactly like the scheduled run: on success `lastRunAt` + `lastSuccessAt` are stamped, `lastError` is explicitly nulled and `consecutiveFailures` reset, so the Upozornění → Úlohy row clears its **error** *and* its **overdue** state immediately. Before this the endpoints called the service functions directly — `runJob()` is the only writer of `jobRuns` — so a successful re-run returned 200 while the tab stayed red until the next scheduled execution, which for a daily job meant up to 24 h of false alarm.

⚠️ Two constraints if you add another trigger: `runJob()` **re-throws** (deliberately, so Cloud Functions error reporting and the platform retry still see the failure), and an escaping rejection in an Express 4 async handler hangs the request — so convert it to a JSON 500, which `runManualTrigger` does. And `runJob()` resolves to `void` while the endpoints return the job's result as JSON, so the result is captured by closure rather than by widening `runJob`'s signature. The vacation-rollover **dry run** is deliberately NOT recorded: it writes nothing, and previewing a plan must not clear a real failure off the tab.

**In-app UI — Settings → Úlohy** (`frontend/src/pages/settings/JobsTab.tsx`, tab id `"jobs"`): **ten** buttons as of v5.11.6 — nine in the `JOBS` array plus the vacation rollover, which is rendered separately because it runs as a confirmed dry-run first. ⚠️ `JOBS` is a **hardcoded list independent of `JOB_REGISTRY`** in `functions/src/services/jobRuns.ts`: the registry's `triggerEndpoint` only drives the Upozornění → Úlohy *hint text*, while the actual buttons live here. Setting one without the other yields a hint pointing at a button that does not exist. A card-per-job panel that POSTs each endpoint with a `ConfirmModal` confirmation before firing. Visible to users holding `system.triggers` (tab is registered in `SETTINGS_TABS` in `SettingsPage.tsx`). Each card shows a one-line description and a result/error badge after the run. Use this instead of curl for day-to-day manual re-runs.
