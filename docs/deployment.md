...
Several `POST /api/.../trigger-*` endpoints mirror scheduled jobs (publish shift plans, sweep Multisport, refresh probation / document alerts, re-fold employees' effective root fields, sweep Recepce history, run scheduled deactivations). They exist so an admin can re-run a job after a missed scheduled execution (and to backfill data after a fix):
...
All are gated `requireAuth` + `requirePermission("system.triggers")` and write a `manual-trigger` audit entry per successful call (`extra.trigger` names the underlying job, `extra.result` carries the job's return value). The audit-log write happens *after* the job, so a failed re-run leaves no entry — the failure surfaces in `firebase functions:log`. Only the first five are wired into the **Settings → Úlohy** in-app UI below — the deactivation-sweep and Recepce-retention triggers are backend/testing-only today (curl or a future `JobsTab.tsx` addition).
