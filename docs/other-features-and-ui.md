...
- **Sections:**
  ...
  - **MOD** — letter from `modRow/{todayYMD}.code`. The letter resolves to an employee through `plan.modPersons` (letter → employeeId) via `modEmployeeIdForLetter()` in `lib/modPersons.ts`. There is no name-based fallback: the static `MOD_PERSONS` table was removed in v4.7.1 (see `docs/shifts.md`), so a letter nobody holds in this plan simply renders as "Nepřiřazeno".
...
Managed in Settings → Seznamy tab, Společnosti section. Only one card in edit mode at a time.
...
**Hidden entries (`isHiddenEntry`).** All three read paths (list, `:id`, `meta/collections`) drop a small set of historical entries by design; see the function and `docs/recepce.md` → "Souhrn recepce". Keep the guard.
...
**End date = last in-probation day, not the first day after (v4.8.2).** `computeProbationEndDate(startDate, parsed)` used to return the day probation *ends* (`start + duration`); it now returns the **last day the employee is still in probation** (`start + duration − 1 day`, e.g. a 2-month probation from 2026-07-15 now reads "Konec zkušební doby: 2026-09-14", not 2026-09-15). Probation covers the half-open range `[start, start + duration)`, so this is the same boundary the frontend's termination-template picker uses to decide "in probation" — see [Contracts & Templates — Probation-aware termination template selection (v4.8.2)](contracts.md#probation-aware-termination-template-selection--dpp-fix-v482). Before this fix the two could disagree by one day at the boundary (the alert said "ends the 15th" while the auto-selected termination template already treated the 15th as past probation). Week/day-based durations get the same `− 1 day` treatment.
...
- **`frontend/src/components/MonthNav.tsx`** (v5.8.1) — the `‹ / › + month label + DNES` pager shared by the shift planner and payroll. Owns the year-wrapping paging itself, so a host page holds only `selectedYear`/`selectedMonth`; props are `year`, `month`, `onSelect(y, m)`, `today`, optional `dataTour`. ⚠️ `today` must be `clock.now()`, never `new Date()`, or DNES ignores the test clock. DNES is always rendered and merely `disabled` on the current month — hiding it changes the row's width and makes the centred label jump. Its `≤560px` condensing rule lives in `MonthNav.module.css`, **not** in the host page: the class names it targets are the component's, so a page-side copy would silently stop matching.
- **`frontend/src/components/MonthJumpSelect.tsx`** (v5.8.0/5.8.1, mobile rule v5.8.2) — quick-jump `<select>` over every month that exists, grouped by year (`<optgroup>`), newest first. Pairs with `MonthNav` in the header's `1fr auto 1fr` grid (picker left, nav centred). Fed by whatever list the host already has: `plansList` on the shift planner (polled anyway for change detection), `GET /payroll/periods` on payroll (cheap — `entries` is a sub-collection, so each period doc is a few hundred bytes). Value is `""` with a disabled placeholder when the selected month has no entry, which is legitimate on both pages. ⚠️ With no items it still renders its wrapper `<div>` and omits only the `<select>`; returning `null` would drop a grid child and knock the centred month label off-centre. **Hidden entirely on phones / short landscape** (v5.8.2): both host `.header` grids collapse to a single `1fr` column at that breakpoint, so the picker stopped being a left-hand slot and became a full-width row stacked above the month label; `‹ / › + DNES` is the only month navigation on mobile. The rule hides the whole `.wrap`, not just `.select` — the wrapper is a grid item, and an empty one leaves a phantom row in the stacked layout. It lives in `MonthJumpSelect.module.css` at the same breakpoint `MonthNav` uses, per the same file-scoping caveat noted above.
...
- **`.shell` + `.shellBody`** — the preferred shape, for a modal built as header/body/footer. `.shell` caps the box (`max-height: calc(100vh - 3rem)`) and turns it into a column flex container; `.shellBody` (composed onto the scrolling middle element) adds `overflow-y: auto`. The header and footer, left at their default `overflow: visible`, are floored at their own content height and stay pinned and reachable — only the body gives way. **`overflow-y: auto` is what actually does the work here**: per spec, a flex item's automatic minimum size drops to zero the moment its overflow is anything other than `visible`, which is what lets `.shellBody` shrink below its content at all and is why the header/footer end up pinned rather than also being compressed.
- **`.shellScroll`** — the fallback shape, for a dialog whose title/fields/actions are plain siblings with no body wrapper to scroll (same `max-height` + `overflow-y: auto` on the whole box). The footer buttons travel with the content instead of staying pinned, but everything stays reachable — which is the point. `SettingsPage.tsx`'s 14 dialogs share one `.modalBox` with no body wrapper, so capping that one shared box fixed all 14 at once; restructuring each into header/body/footer to use `.shell` instead would have fixed one dialog at a time for the same fourteen-times cost.

Consumers today: `ConfirmModal.module.css` (`.shell`/`.shellBody`), `SignModal.module.css`, `frontend/src/pages/recepce/HandoverTab.module.css`, `OdvodyModal.module.css`, `TerminalTab.module.css` (all `.shell`/`.shellBody`), and `SettingsPage.module.css` (`.shellScroll`).

**Why `composes` and not a lint rule:** `frontend/` has no ESLint or Stylelint config at all (no `.eslintrc*`/`eslint.config.*`, no `.stylelintrc*`, and neither package is a `frontend/package.json` dependency) — introducing a rule to catch a hand-rolled, uncapped modal would mean introducing an entire toolchain for one convention. Composing the shared class is the lint rule, enforced by CSS Modules at build time (a typo in the `composes` path or the class name fails the build) rather than by a linter.

**The `SignModal` case is the instructive one.** Its body and footer sit inside a `<form>`, and a plain block-level `<form>` sizes to its content — so `.shell`'s cap never reached `.body` in the first place, and `overflow: hidden` on `.modal` (kept for the rounded corners) clipped the footer buttons with **no scrollbar at all** to reveal that anything was cut off. The fix was making the `<form>` itself the flex column (`display: flex; flex-direction: column`) with `min-height: 0`. This is the one place in the whole convention where `min-height: 0` is actually load-bearing — it is what lets `.form` (and therefore `.body` beneath it) shrink inside `.modal` at all. Everywhere else in this convention the shrinking comes from `.shellBody`'s `overflow-y: auto` alone, per the spec rule above — do not generalise "`min-height: 0` matters" beyond this one form-wrapped case.

⚠️ **Landmine — a modal with an absolutely-positioned popover.** `AddEmployeeToPlanModal.module.css` is the one modal in the app whose body contains a `position: absolute` element (`.dropdown`, an autocomplete suggestion list anchored under `.searchWrapper`). It is already correctly capped (a hand-written equivalent of the `.shell`/`.shellBody` shape: `.modal` is `max-height: 85vh` + column flex, `.body` is `flex: 1; overflow-y: auto`). The landmine is for *next* time someone "helpfully" restructures this or a similar modal: wrapping the popover's positioned ancestor in a *second*, narrower inner scroll container (rather than relying on the one scrolling body it already has) would clip the dropdown the moment it needed to render past that container's edge, with no visual cue that anything was cut off — the same silent-clip failure mode `.shellBody` exists to prevent, just one level deeper. When capping a modal that contains a positioned popover, keep the popover's scrolling ancestor to the outermost body wrapper, not a nested one.
...
## Settings — Seznamy tab consolidation (v4.8.1)

The four former settings tabs — **Společnosti**, **Oddělení**, **Pracovní pozice**, **Vzdělání** — are merged into a single **Seznamy** tab in `frontend/src/pages/SettingsPage.tsx` (`SettingsPage.module.css`). Each of the four is now a collapsible section within that one tab, rendered via a new local `CollapsibleSection` component (title + chevron toggle + optional header `action` button; body unmounts when collapsed). Sections default **collapsed**.

- **Tab-level gating:** the Seznamy tab is shown when the user holds **any** of the four permissions — `SEZNAMY_PERMS = [settings.companies.manage, settings.departments.manage, settings.jobPositions.manage, settings.educationLevels.manage]`, checked via `canManageSeznamy`. **Section-level gating** is unchanged — each section still individually checks its own permission (e.g. a user with only `settings.departments.manage` sees the tab with just the Oddělení section).
- **"+ Přidat …" actions** moved into each section's collapsible header (`action` prop) instead of a page-level toolbar.
- **The five create/cascade modals were relocated to the page's top level** — company create, department create, position create, position-cascade, and education create now render alongside the page's existing delete modals, outside any `CollapsibleSection`. This is required because a collapsed section unmounts its body (`{open && <div>{children}</div>}`); a modal that lived inside the section's JSX would vanish mid-interaction if its section collapsed.
- The `SettingsTab` union type changed from four separate tab ids to one `"seznamy"` id. `SETTINGS_TABS` special-cases it — its listed `perm` (`settings.companies.manage`) is nominal for display-order purposes only; actual visibility uses `canManageSeznamy`, not a single `can(perm)` check.
- The four underlying Firestore collections (`companies`, `departments`, `jobPositions`, `educationLevels`) and their permissions are unchanged — only how they're grouped in the Settings UI changed. The tour (`appTour.ts`) and Nápověda already point at `data-tour="settings-tab-seznamy"` and describe "the Seznamy tab, … section" — see [Onboarding Tour & Nápověda](onboarding-and-help.md).
...
- **Settings tab order** — the Settings page tab strip now starts with **Uživatelé** → **Uživatelské typy** (users first, then types), followed by **Seznamy** (as of v4.8.1 a single tab consolidating the former Společnosti/Oddělení/Pracovní pozice/Vzdělání tabs — see "Settings — Seznamy tab consolidation" above), then Mzdy/Menu/Úlohy. The order is driven by the render order in `SettingsPage.tsx`.
